...
class Ajax_pagination_gal2{
	...
	function create_links(){
		...
		if ($this->cur_page < $num_pages) {
			$output .= $this->loadLink_gal($this->cur_page * $this->per_page);
		}else{
			$output .= $this->loadLink_gal(0);
		}
        ...
        function getData_gal2(){  
        	...
        	var page = $('#pageGAL2').val();
            $.ajax({
                method: "POST",
                url: "<?php echo $this->base_url; ?>"+page,
                data: { pageGAL2: page },
                beforeSend: function(){
                    $('<?php echo $this->loading; ?>').slideDown();
                },
                success: function(data){
                   	$('<?php echo $this->target; ?>').animate({ marginLeft: "100%"},350);

                	$('<?php echo $this->target; ?>').html(data).animate({ marginLeft: 0},350);
	...
	function getAJAXlink_gal($count, $text) {
        $pageCount = $count?$count:0;
		return $text.' ';
	}

	function loadLink_gal($page){
		return '<input type="hidden" value="'.$page.'" name="pageGAL2" id="pageGAL2">';
