...
<html>
  <head>
	...
	<style>
		...
		.tanggal{ background: #82231f no-repeat left top; position:fixed;right:0px;bottom:0;font-size: 16px;height:40px;width:115px;margin-bottom: 0px;padding: 13px 10px 10px 15px;font-weight: bold;z-index: 999999999;color:#fff;}
...
<body class="skin-blue sidebar-mini" style="background: <?php echo $par['sch_warna_latar']?>">
			...
			<div class="col-lg-12" style="font-family: Arial !important; padding-top: 10px;">
				<div class="box box-default" style="height:<?php echo $par['height_agenda_1']?>px;">
					<div class="box-header with-border box-agenda">
	              	...
	              	</div>		<div class="blink_me badge" style="color:#fff;margin-top:-38px; font-size:22px;position:absolute;right:2%;padding:5px;background: none !important;"> <?php echo $total_rowsx; ?> </div>
							...
							<div class="postPST" id="postPST">
								...
								<script>
							        function getData(){  
							            ...
							            $.ajax({
							                method: "POST",
							                url: "<?php echo site_url('tigris/Ids_jadwal/ajaxPaginationData'); ?>/"+page,
							                data: { page: page },
							                beforeSend: function(){
	...
	<script type="text/javascript">
	  $(document).ready(function() {
	  	...
	  	$.ajax({
			url : "<?php echo site_url('tigris/Ids_jadwal/ajaxPaginationData');?>",
			// dataType : "JSON",
			success : function(data){
	  ...
	  function tgl_reload(){
	    $.ajax({
	      url: "<?php echo site_url('tigris/Ids_jadwal/ajax_tgl') ?>",
	      dataType:'JSON',
	      success:function(data){
	...
	<script type="text/javascript">
		...
		function get_durasi_sc(durasi, st_data, tot){
			$.ajax({
				url: '<?php echo site_url('tigris/Ids_jadwal/get_durasi_sc') ?>/'+durasi,
				dataType:"JSON",
				cache: false,
				success: function(data) {
					$('#durasi_sc').html(data); 
		...
		function teks_bergerak() {
			...
			$.ajax({
				url: '<?php echo site_url('tigris/Ids_jadwal/teksbergerak') ?>',
				cache: false,
