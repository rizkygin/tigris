...
class Login extends CI_Controller {
	...
	function login_process() {
        ...
        // end-tnde
        */
		$where = array('username' => $username, 'password' => md5($password)/*, 'status_aktif' => 1*/);
		...
		$set = $this->general_model->datagrab(array(
			'tabel' => array(
				...
				'ref_unit cc' => array('p.id_unit=cc.id_unit','left'),
				'ref_bidang vv' => array('p.id_bidang=vv.id_bidang','left'),
     ...
     die();*/
		if ($set->num_rows() > 0) {
			...
			if ($roled->num_rows() > 0) {
				$cek_status_aktif = $this->general_model->datagrabs(array(
					'tabel' =>'peg_pegawai',
					'where' => array('id_pegawai' => $user->id_pegawai)
				))->row();
			
				/*$aplikasi_pindah=die(json_encode(array("sign" => "3","teks" => $cek_status_aktif->status_aktif)));*/
				if($cek_status_aktif->status_aktif == 0){
						$aplikasi_pindah=die(json_encode(array("sign" => "3","teks" => "Pengguna di non aktifkan,<br>hubungi Administrator!")));
					}else{
