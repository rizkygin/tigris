...
class Ids_jadwalx extends CI_Controller {
...
// ./ GALERI FOTO 1
		...
		//cek data email
		$cek_email = $this->general_model->datagrabs(array('tabel' => 'sch_email','order'=>'nama_pemilik ASC'));
		$g = array();
		foreach($cek_email->result() as $a){
			$g[] = $a->nama_email.',';
	
		}	

		//cek data email
		$jadwal_besok = $this->general_model->datagrabs(array(
			'tabel'=>$from,
			'select'=>$select,
			'where'=>array('tj.tgl_mulai'=>date('Y-m-d', time() + 86400))
			));
		/*			cek($jadwal_besok->num_rows());
					die();*/
		if($jadwal_besok->num_rows() > 0){

			
			$from_notif = array(
				'sch_notif a' => '',
				'sch_jadwal b' => array('b.id_jadwal = a.id_jadwal','left')
			);
			$cek_notif = $this->general_model->datagrab(array(
				'tabel'=>$from_notif,
				'where'=>array('a.date_masuk'=>date('Y-m-d', time() + 86400))
			));
			/*cek($jadwal_besok->num_rows());
			cek($cek_notif->num_rows());
			die();*/
			if($jadwal_besok->num_rows() != $cek_notif->num_rows()){
				if($cek_notif->num_rows() > 0){
						$delete_data=$this->general_model->delete_data(array('tabel'=>'sch_notif','where'=>array('date_masuk'=>date('Y-m-d', time() + 86400))));
					$xxx = array();
					foreach ($jadwal_besok->result() as $i => $result) {
							
						$xxx[$i] = $result->id_jadwal;
							$param_inv = array(
								'tabel'=>'sch_notif',
								'data'=>array(
									'id_jadwal'=>$result->id_jadwal,
									'date_masuk'=>$result->tgl_mulai,
									'status'=>0
									));
							$simpan_selesai=$this->general_model->save_data($param_inv);
							

							$email_set = $this->general_model->datagrab(array('tabel'=>'sch_email_setting', 'where'=>array('id_email_setting'=>1)))->row();
							$config = array(
							  'protocol' => $email_set->protocol,
							  'smtp_host' => $email_set->smtp_host,
							  'smtp_port' => $email_set->smtp_port,
							  'smtp_user' => $email_set->email, // change it to yours
							  'smtp_pass' => $email_set->password, // change it to yours
							  'mailtype' => $email_set->mail_type,
							  'charset' => $email_set->charset,
							  'wordwrap' => TRUE
							);

							$this->load->library('email', $config);
							$this->email->set_newline("\r\n");
							$this->email->from($email_set->email, $email_set->username);
							//$this->email->to('video.yesjogja@gmail.com'); 
							$this->email->to(substr(implode('',$g), 0, -1)); 
							$this->email->subject('E-Scheduling - '.$result->nama_rincian.' ('.tanggal_indo($result->tgl_mulai).' s.d '.tanggal_indo($result->tgl_selesai).')');
							$this->email->message('
								<table>
								<tr>
									<td><b>Program </b></td>
									<td><b> : </b></td>
									<td><b>'.$result->nama_program.'</b></td>
								</tr>
								<tr>
									<td><b>Kegiatan </b></td>
									<td><b> : </b></td>
									<td><b>'.$result->nama_kegiatan.'</b></td>
								</tr>
								</table>
								<br>
								<p>'.$result->content.'</p>
								...
								<br><br><br>
								*Teriimakasih atas perhatiannya'
								);
								if($this->email->send()){
								$param_not = array(
									'tabel'=>'sch_notif',
									...
									'where'=>array('date_masuk'=>$result->tgl_mulai)
									);
								$simpan_selesai=$this->general_model->save_data($param_not);
								}else{
...
/*
		...
		$data['data_email']=substr(implode('',$g), 0, -1);
*/
		//$data['data_email']= substr(implode('',$g), 0, -1);
		//$data['data_email']= substr(implode('',$dat_jadwal_besok), 0, -1);
		//$data['data_email']= $jadwal_besok->num_rows();
		$data['data_email']= $jadwal_besok->num_rows();
		if($status1->id_theme == 1){
		...
		if ($detail_kegiatan->num_rows() > 0) {
			...
			foreach ($detail_kegiatan->result() as $row) {
				...
				if(strlen($row->nama_rincian) >= 40){
					...
					$rows[] = 	'<div class="luarx"><div class="dalamx">'.$row->nama_rincian.'</div></div>';
				}else{
					$rows[] = 	array('data'=>$row->nama_rincian);
				}
				if($row->tgl_mulai != NULL){
			...
			$(document).ready(function() {
				//$(\'.luarx\').height($(\'.box-penxg\').height()-130);
				var ombopengumuman = $(\'.luarx\').height()-0;
				$(\'.dalamx\').each(function() {
						set_peng($(this));
				});
				//alert(ombopengumuman);
				// alert(ombopengumuman);
						...
						setInterval(function() {
							if (parseInt($(from).parent().css(\'height\')) < parseInt($(from).css(\'height\'))) {
								...
								if (parseInt(posnow) > (-0.5)*parseInt($(from).css(\'height\'))) {
									...
									$(from).css(\'top\',(parseInt(posnow)-ombopengumuman)+\'px\')
								} else {
									$(from).animate({ \'top\': 0});
								}
							}
