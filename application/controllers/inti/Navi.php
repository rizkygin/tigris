...
class Navi extends CI_Controller {
	...
	function tabel($r,$a) {
		...
		$nav = $this->general_model->datagrab(array(
			'tabel'=> 'nav',
			'where' => array('ref = '.$r.' AND (id_par_nav IS NULL OR id_par_nav=0 ) and id_aplikasi = "'.$a.'"' => null),
			'order' => 'urut',
		));
	...
	function show_navi($par = null) {
		...
		$active = (isset($apps->aktif) and $apps->aktif > 0) ? '<span class="label label-info" style="font-weight: 100; letter-spacing: 2px">Aktif</span>' : '<span class="label" style="font-weight: 100; letter-spacing: 2px">Non-Aktif</span>';
		$data['title']	= 'Pengaturan Navigasi '.@$apps->nama_aplikasi.' '.$active;
		$where_ref =  ($par['ref'] == 2) ? null : "folder != 'referensi' AND ";
		
		$app_combo_data = $this->general_model->datagrab(array('tabel' => 'ref_aplikasi','order' => 'urut','where' => array($where_ref.'(id_par_aplikasi IS NULL OR id_par_aplikasi=0)' => null)));
