...
class Akses extends CI_Controller {
	...
	function data_list($d,$offset = null) {
	
		$data['breadcrumb'] = array('' => $this->in_app, $d['dir'].'/kewenangan' => 'Kewenangan');
		...
		$where = ($d['app'] == 1) ? 
			array('a.folder' => $d['dir']) : 
			array('a.id_aplikasi IN ('.implode(',',$app_active).')' => null);
		$config['base_url']		= site_url($d['dir'].'/kewenangan/list_kewenangan');
		...
		if ($dtkewenangan->num_rows() != 0) {
			...
			foreach($dtkewenangan->result() as $row){
				$link1 = anchor('#','<i class="fa fa-pencil"></i>','class="btn btn-xs btn-warning btn-edit" act="'.site_url($d['dir'].'/kewenangan/add_kewenangan/'.$row->id_role).'"');
				$link2 = anchor($d['dir'].'/kewenangan/delete_kewenangan/'.$row->id_role,'<i class="fa fa-trash"></i>','class="btn btn-xs btn-danger btn-delete" msg="Apakah Anda ingin menghapus kewenangan <b>'.$row->nama_role.'</b>?"');
				...
				$conf = '<div class="btn-group">
						...
						<li>'.anchor($d['dir'].'/kewenangan/set_kewenangan/'.in_de(array('set' => 1,'id' => $row->id_role)),'<i class="fa fa-sign-in fa-btn"></i> Set Semua Kewenangan ke Operator','class="btn-delete" '.$opt_set).'</li>
						<li>'.anchor($d['dir'].'/kewenangan/set_kewenangan/'.in_de(array('set' => 2,'id' => $row->id_role)),'<i class="fa fa-sign-out fa-btn"></i> Cabut Semua Kewenangan ke Operator','class="btn-delete" '.$opt_unset).'</li></ul>
						</div>';
		...
		$data['tombol'] = anchor('#','
			...
			class="btn-edit btn btn-success btn-flat" act="'.site_url($d['dir'].'/kewenangan/add_kewenangan').'"');
		}
	...
	function add_kewenangan($d,$id_role = null){
		
		$active = $this->general_model->datagrab(array('tabel' => 'ref_aplikasi', 'where' => array('aktif' => 1)));
	
		$data['breadcrumb'] = array('' => $this->in_app, $d['dir'].'/kewenangan/add_kewenangan/'.$id_role => 'Kewenangan');
		
		$where = ($d['app'] == 1) ? array('folder' => $d['dir']) : array('(id_par_aplikasi IS NULL OR id_par_aplikasi=0)' => NULL);
	...
	function save_kewenangan() {
		...
		redirect($this->input->post('dir').'/kewenangan');
	}
	
	function delete_kewenangan($dir,$id_role){
		$this->general_model->delete_data('ref_role_nav','id_role',$id_role);
