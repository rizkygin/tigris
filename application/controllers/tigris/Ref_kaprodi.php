...
class Ref_kaprodi extends CI_Controller{
    var $dir = 'tigris/Ref_kaprodi';
    ...
    public function list_data($search=NULL, $offset=NULL) {
		$data['breadcrumb'] = array($this->dir => 'Referensi Kepala Prodi');
		$offset = !empty($offset) ? $offset : null;
		$fcari = null;
		$search_key = $this->input->post('key');
		if (!empty($search_key)) {
			$fcari = array(
				'nama_kaprodi' 		=> $search_key,
				'nama_prodi' 		=> $search_key,
				'periode' 		=> $search_key,
			);	
			$data['for_search'] = @$fcari['nama_kaprodi'];
		...
		foreach ($nav->result() as $rowx) {
			$awal[] = array($rowx->id_ref_kaprodi,$rowx->urut);
		}
		...
		if ($dtjnsoutput->num_rows() > 0) {
			...
			foreach ($dtjnsoutput->result() as $row) {
				$rows = array();
				if($row->status == 1){
					$status = anchor('tigris/Ref_pengajuan_judul/on/'.in_de(array('id_ref_kaprodi' => $row->id_ref_kaprodi,'status' =>0)),'<i class="fa fa-fw fa-toggle-on text-aqua" style="font-size:20px;"></i>');
				}else{
					$status = anchor('tigris/Ref_pengajuan_judul/on/'.in_de(array('id_ref_kaprodi' => $row->id_ref_kaprodi,'status' =>1)),'<i class="fa fa-fw fa-toggle-off text-default" style="font-size:20px;"></i>');

				}
				//cek($row->id_ref_pengajuan_judul);
				$btn_down = ($m+1 < $nav->num_rows()) ? anchor('tigris/Ref_pengajuan_judul/urut/'.in_de(array('id1' => $row->id_ref_kaprodi,'no1' => $row->urut,'id2' => $awal[$m+1][0],'no2' =>  $awal[$m+1][1])),'<i class="fa fa-arrow-down"></i>') : ' &nbsp; ';
					$btn_up = ($m > 0) ? anchor('tigris/Ref_pengajuan_judul/urut/'.in_de(array('id2' => $row->id_ref_kaprodi,'no2' => $row->urut,'id1' =>  !empty($urutkan[0])?$urutkan[0]:@$awal[0],'no1' => !empty($urutkan[1])?$urutkan[1]:@$awal[1])),'<i class="fa fa-arrow-up"></i>') : ' &nbsp; ';
					$urutkan = array($row->id_ref_kaprodi,$row->urut);


				$rows[] = 	array('data'=>$no,'style'=>'text-align:center');
				/*$rows[] = 	$row->kode_Syarat Pengajuan Judul;*/
				$rows[] = 	$row->nama_kaprodi;
				$rows[] = 	$row->nip;
				$rows[] = 	$row->nama_prodi;
				$rows[] = 	@$status;
				...
				if (!in_array($offset,array("cetak","excel"))) {
					...
					$ubah = anchor('#','<i class="fa fa-pencil"></i>', 'class="btn btn-xs btn-warning btn-edit btn-flat" act="'.site_url($this->dir.'/add_data/'.in_de(array('id_ref_kaprodi'=>$row->id_ref_kaprodi))).'" title="Ubah data kaprodi..."');
					
					/*$ubah = anchor('#','<i class="fa fa-pencil"></i>','class="btn btn-xs btn-flat btn-warning btn-edit" act="'.site_url($this->dir.'/add_data/'.in_de(array('id_ref_kaprodi'=>$row->id_ref_kaprodi))).'" title="Klik untuk edit data"');*/
					$hapus = anchor('#','<i class="fa fa-trash"></i>','class="btn btn-xs btn-flat btn-danger btn-delete" act="'.site_url($this->dir.'/delete_data/'.in_de(array('id_ref_kaprodi'=>$row->id_ref_kaprodi))).'" msg="Apakah anda yakin ingin menghapus data ini ?" title="klik untuk menghapus data"');
					$rows[] = 	$ubah;
					$rows[] = 	$hapus;
				}
		...
		$btn_tambah = anchor('#','<i class="fa fa-plus fa-btn"></i> Tambah Data Kaprodi', 'class="btn btn-success btn-edit btn-flat" act="'.site_url($this->dir.'/add_data/').'" title="Klik untuk tambah data"');
    ...
    public function add_data($param=NULL){
        ...
        $data = array(
            'button' => 'Tambah',
            'action' => site_url('tigris/ref_kaprodi/save_aksi'),

        'id_ref_kaprodi' => set_value('id_ref_kaprodi'),
	    );

		$data['title'] = (!empty($o)) ? 'Ubah Data kaprodi' : 'Kaprodi Baru';
       	$dt = !empty($id) ?  $this->general_model->datagrab(array(
					'tabel' => 'ref_kaprodi',
					'where' => array('id_ref_kaprodi' => $id)))->row() : null;
		...
		$data['form_data'] = '';
		$data['form_data'] .= '<input type="hidden" name="id_ref_kaprodi" class="id_ref_kaprodi" value="'.$id .'"/>';
		$data['form_data'] .= '<div class="row">';
		$data['form_data'] .= '<div class="col-lg-6">';/*
			$data['form_data'] .= '<label>Kode Kaprodi</label>';
			$data['form_data'] .= form_input('kode_Kaprodi', @$dt->kode_Kaprodi,'class="form-control" placeholder="Kode Kaprodi" required');
			$data['form_data'] .= '<p></p>';*/
			$data['form_data'] .= '<p><label>Nama Kaprodi</label>';
			$data['form_data'] .= form_input('nama_kaprodi', @$dt->nama_kaprodi,'class="form-control" placeholder="Nama Kaprodi" required');
            ...
            $data['form_data'] .= form_dropdown('id_ref_prodi',$cb_prodi,@$dt->id_ref_prodi,'class="combo-box form-control" id="id_ref_kaprodi" style="width: 100%"');
    ...
    function save_aksi(){
    	$id_ref_kaprodi = $this->input->post('id_ref_kaprodi');
		...
		// var_dump($input);
		// die();
    	/*$kode_Kaprodi = $this->input->post('kode_Kaprodi');*/
    	$tgl_skrg = date('Y-m-d');
     	$u = $this->general_model->datagrab(array(
            ...
            if(empty($id_ref_kaprodi)) {
            	
            	$cek_prop = $this->general_model->datagrabs(array('tabel'=>'ref_kaprodi','where'=>array('nama_kaprodi'=>$nama_kaprodi),'select'=>'id_ref_kaprodi, MAX(id_ref_kaprodi) as id'))->row();
                if(empty($cek_prop->id_ref_kaprodi)) {
                	$id_prop = $this->general_model->save_data('ref_kaprodi',array('nama_kaprodi' => $nama_kaprodi,'periode' => $periode, 'id_ref_prodi' => $id_prodi, 'nip' => $nip,'urut' => $urut,'status' => 1));
	                $this->session->set_flashdata('ok', 'Data Berhasil Disimpan...');
	            }else{
	                $id_prop = $cek_prop->id_ref_kaprodi ;
	                $this->session->set_flashdata('fail', 'Nama Kaprodi sudah ada...');
	            }
            }else{
            	...
            	$par = array(
					'tabel'=>'ref_kaprodi',
					'data'=>array(
						'nama_kaprodi'=>$nama_kaprodi,'urut' => $urut,
						'periode' => $periode, 'id_ref_prodi' => $id_prodi, 'nip' => $nip
						),
					);

					$par['where'] = array('id_ref_kaprodi'=>$id_ref_kaprodi);
	...
	function delete_data($code) {
		$sn = un_de($code);
		$id_ref_kaprodi = $sn['id_ref_kaprodi'];
		$del = $this->general_model->delete_data('ref_kaprodi','id_ref_kaprodi',$id_ref_kaprodi);
		if ($del) {
			$this->session->set_flashdata('ok','Data Kaprodi Berhasil di Hapus');
		}else{
