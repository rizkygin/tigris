...
class Ref_proposal_tesis extends CI_Controller {
	var $dir = 'tigris/Ref_proposal_tesis';
	...
	public function list_data($search=NULL, $offset=NULL) {
		$data['breadcrumb'] = array($this->dir => 'Referensi Syarat Proposal Tesis');
		...
		if ($dtjnsoutput->num_rows() > 0) {
			$heads = array('No','Nama Syarat Proposal Tesis','Keterangan','di Tujukan ke','Tipe Field','Status');
			...
			foreach ($dtjnsoutput->result() as $row) {
				...
				/*$rows[] = 	$row->kode_Syarat Proposal Tesis;*/
				...
				if (!in_array($offset,array("cetak","excel"))) {
					//$ubah = anchor(site_url($this->dir.'/add_data/'.in_de(array('id_ref_proposal_tesis'=>$row->id_ref_proposal_tesis))), '<i class="fa fa-pencil"></i>', 'class="btn btn-xs btn-flat btn-warning" ');
					$ubah = anchor('#','<i class="fa fa-pencil"></i>', 'class="btn btn-xs btn-warning btn-edit btn-flat" act="'.site_url($this->dir.'/add_data/'.in_de(array('id_ref_proposal_tesis'=>$row->id_ref_proposal_tesis))).'" title="Edit Data Agenda Kegiatan..."');
					
					/*$ubah = anchor('#','<i class="fa fa-pencil"></i>','class="btn btn-xs btn-flat btn-warning btn-edit" act="'.site_url($this->dir.'/add_data/'.in_de(array('id_ref_proposal_tesis'=>$row->id_ref_proposal_tesis))).'" title="Klik untuk edit data"');*/
					$hapus = anchor('#','<i class="fa fa-trash"></i>','class="btn btn-xs btn-flat btn-danger btn-delete" act="'.site_url($this->dir.'/delete_data/'.in_de(array('id_ref_proposal_tesis'=>$row->id_ref_proposal_tesis))).'" msg="Apakah anda yakin ingin menghapus data ini ?" title="klik untuk menghapus data"');
					$rows[] = 	$ubah;
					$rows[] = 	$hapus;
				}
...
/*
		$btn_tambah = anchor('#','<i class="fa fa-plus fa-btn"></i>Nama Syarat Proposal Tesis', 'class="btn btn-success btn-edit btn-flat" act="'.site_url($this->dir.'/add_data').'" title="Klik untuk tambah data"');*/
		//$btn_tambah = anchor(site_url($this->dir.'/add_data'), '<i class="fa fa-plus"></i> Nama Syarat Proposal Tesis', 'class="btn btn-md btn-success btn-flat"');
		
		$btn_tambah = anchor('#','<i class="fa fa-plus fa-btn"></i> Tambah Syarat Proposal Tesis', 'class="btn btn-success btn-edit btn-flat" act="'.site_url($this->dir.'/add_data/').'" title="Klik untuk tambah data"');
		...
		$title = 'Referensi Syarat Proposal Tesis';
	...
	function on($par) {
		...
		$param1 =
			...
			$param1['where'] = array('id_ref_proposal_tesis'=>$o['id_ref_proposal_tesis']);
	...
	function urut($par) {
		...
		$param1 =
			...
			$param1['where'] = array('id_ref_proposal_tesis'=>$o['id1']);
			$this->general_model->simpan_data($param1);
			/*die();*/
		...
		$param2 =
			...
			$param2['where'] = array('id_ref_proposal_tesis'=>$o['id2']);
			 $this->general_model->simpan_data($param2);




		//$this->general_model->save_data('ref_proposal_tesis',array('urut' => $o['no2']),'id_ref_proposal_tesis',$o['id1']);
		//$this->general_model->save_data('ref_proposal_tesis',array('urut' => $o['no1']),'id_ref_proposal_tesis',$o['id2']);
		redirect($this->dir);
    ...
    public function add_data($param=NULL){
    	$o = un_de($param);
    	$id= @$o['id_ref_proposal_tesis'];
        $data = array(
            'button' => 'Tambah',
            'action' => site_url('tigris/ref_proposal_tesis/save_aksi'),

        'id_ref_proposal_tesis' => set_value('id_ref_proposal_tesis'),
		);
        $from = array(
			'ref_proposal_tesis a' => '',
			'ref_tipe_field b' => array('a.id_ref_tipe_field = b.id_ref_tipe_field','left')
		);
		$data['title'] = (!empty($code)) ? 'Ubah Data Syarat Proposal Tesis' : 'Syarat Proposal Tesis Baru';
       	$dt = !empty($id) ?  $this->general_model->datagrab(array(
					'tabel' => $from,
					'where' => array('a.id_ref_proposal_tesis' => $id)))->row() : null;
		...
		$data['form_data'] = '';
		$data['form_data'] .= '<input type="hidden" name="id_ref_proposal_tesis" class="id_ref_proposal_tesis" value="'.$id .'"/>';
		$data['form_data'] .= '<div class="row">';
		$data['form_data'] .= '<div class="col-lg-6">';
			$data['form_data'] .= '<p><label>Nama Syarat Proposal Tesis</label>';
			$data['form_data'] .= form_textarea('nama_syarat', @$dt->nama_syarat,'class="form-control" placeholder="Nama Syarat Proposal Tesis" required');
			...
			$data['form_data'] .= form_input('keterangan_proposal_tesis', @$dt->keterangan_proposal_tesis,'class="form-control" placeholder="Nama Syarat Proposal Tesis" required');
			$data['form_data'] .= '</div>';
		$data['form_data'] .= '</div>';
    ...
    function save_aksi(){
    	$id_ref_proposal_tesis = $this->input->post('id_ref_proposal_tesis');
    	...
    	$keterangan_proposal_tesis = $this->input->post('keterangan_proposal_tesis',TRUE);
    	/*$kode_Syarat Proposal Tesis = $this->input->post('kode_Syarat Proposal Tesis');*/
    	$tgl_skrg = date('Y-m-d');
     	$u = $this->general_model->datagrab(array(
            ...
            if(empty($id_ref_proposal_tesis)) {
            	
            	$cek_prop = $this->general_model->datagrabs(array('tabel'=>'ref_proposal_tesis','where'=>array('nama_syarat'=>$nama_syarat),'select'=>'id_ref_proposal_tesis, MAX(id_ref_proposal_tesis) as id'))->row();
                if(empty($cek_prop->id_ref_proposal_tesis)) {
                	$id_prop = $this->general_model->save_data('ref_proposal_tesis',array('nama_syarat' => $nama_syarat,'id_ref_tipe_field' => $id_ref_tipe_field,'id_bidang' => $id_bidang,'keterangan_proposal_tesis' => $keterangan_proposal_tesis,'wajib_isi' => $wajib_isi,'urut' => $urut,'status' =>1));
	                $this->session->set_flashdata('ok', 'Data Berhasil Disimpan...');
	            }else{
	                $id_prop = $cek_prop->id_ref_proposal_tesis ;
	                $this->session->set_flashdata('fail', 'Nama Syarat Proposal Tesis sudah ada...');
	            }
            }else{
            	...
            	$par = array(
					...
					'data'=>array(
						...
						'keterangan_proposal_tesis' => $keterangan_proposal_tesis,
						'wajib_isi'=>$wajib_isi
						),
					);

					$par['where'] = array('id_ref_proposal_tesis'=>$id_ref_proposal_tesis);
	...
	function delete_data($code) {
		$sn = un_de($code);
		$id_ref_proposal_tesis = $sn['id_ref_proposal_tesis'];
		$del = $this->general_model->delete_data('ref_proposal_tesis','id_ref_proposal_tesis',$id_ref_proposal_tesis);
		if ($del) {
