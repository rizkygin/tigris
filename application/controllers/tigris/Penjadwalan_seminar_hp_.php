...
class Penjadwalan_seminar_hp extends CI_Controller {
	...
	public function list_data($search=NULL, $offset=NULL) {
		...
		$select = 'a.*,b.nama as nama_mahasiswa,e.nama as xx,f.nama as xxx,c.nama_program_konsentrasi';
		...
		$dtjnsoutput = $this->general_model->datagrabs(array('tabel'=>$from, 'order'=>'id_seminar_hp ASC', 'limit'=>$lim, 'offset'=>$offs, 'search'=>$fcari, 'select'=>$select,'where'=>$where));
...
*/
    ...
    // Line break
        ...
        $pdf->SetX(25);
         $pdf->Image(base_url()."assets/images/corp/ba-shp.png",70,40,80,0);
        ...
        $pdf->SetTextColor(0,0,0);
        $pdf->SetX(25);
        $pdf->MultiCell(180,5,'Pada hari ini,    '.konversi_tanggal('D',substr($dt_row->tgl_mulai,0,10)).'  tanggal    '.konversi_tanggal('j',substr($dt_row->tgl_mulai,0,10)).'   Bulan    '.konversi_tanggal('M',substr($dt_row->tgl_mulai,0,10)).'  Tahun    '.konversi_tanggal('Y',substr($dt_row->tgl_mulai,0,10)).'    , mulai pukul 10.00 WIB sampai 11.00 WIB telah dilaksanakan Seminar Hasil Penelitian Tesis Mahasiswa Program Studi Magister Ilmu Hukum, Fakultas Hukum - UNDIP,');
...
;
        ...
        $pdf->SetTextColor(0,0,0);
        $pdf->SetX(25);
        $pdf->Image(base_url()."assets/images/corp/ba-1.png",20,NULL,180,0);
		...
		$pdf->MultiCell(180,5,'Panitia Seminar Hasil Penelitian Tesis');
				...
				$pemb = $this->general_model->datagrab(array(
			        ...
			        $pdf->SetX(25);
			        $pdf->SetFont('Times','',12);
			        $pdf->MultiCell(300,8,'Ketua Program Studi Magister Ilmu Hukum : Dr. Joko Setiyono, S.H., M.Hum');
        ...
        $pdf->SetTextColor(0,0,0);
        $pdf->SetX(38);
        $pdf->Cell(20,8,'Megister Ilmu Hukum');
        ...
        $pdf->SetTextColor(0,0,0);
        $pdf->SetX(25);
        $pdf->Image(base_url()."assets/images/corp/xx.png",0,NULL,210,0);
        ...
        $pdf->SetX(60);
        $pdf->SetFont('Times','',12);
        $pdf->MultiCell(200,5.8,'Magister Ilmu Hukum');
        ...
        $pdf->SetX(60);
        $pdf->SetFont('Times','',12);
        $pdf->MultiCell(180,5.8,konversi_tanggal('H',substr($dt_row->tgl_mulai,11,19)).':'.konversi_tanggal('i',substr($dt_row->tgl_mulai,11,19)).' WIB - Selesai /*'.konversi_tanggal('H',substr($dt_row->tgl_mulai,11,19)).':'.konversi_tanggal('i',substr($dt_row->tgl_mulai,11,19)).' WIB*/');
        ...
        $pdf->SetTextColor(0,0,0);
        $pdf->SetX(35);
        $pdf->Image(base_url()."assets/images/corp/draft_tesis.png",6,NULL,185,0);
        ...
        $pdf->SetTextColor(0,0,0);
        $pdf->SetX(35);
        $pdf->Image(base_url()."assets/images/corp/coret_yg.png",10,NULL,150,0);
...
;
        ...
        $pdf->SetTextColor(0,0,0);
        $pdf->SetX(35);
        $pdf->Image(base_url()."assets/images/corp/daftar_hadir.png",6,NULL,200,0);
...
$pdf->WriteHTML($html);
        ...
        $pdf->SetTextColor(0,0,0);
        $pdf->SetX(35);
        $pdf->Image(base_url()."assets/images/corp/daftar_hadir.png",0,NULL,210,0);
        ...
        $pdf->SetTextColor(0,0,0);
        $pdf->SetX(35);
        $pdf->Image(base_url()."assets/images/corp/komentar_saran_shp.png",15,NULL,180,0);
...
;
        ...
        $pdf->SetTextColor(0,0,0);
        $pdf->SetX(35);
        $pdf->Image(base_url()."assets/images/corp/peserta_shp.png",15,NULL,180,0);
