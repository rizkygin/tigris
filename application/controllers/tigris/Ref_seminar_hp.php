...
class Ref_seminar_hp extends CI_Controller {
	...
	public function list_data($search=NULL, $offset=NULL) {
		$data['breadcrumb'] = array($this->dir => 'Referensi Syarat Seminar Hasil Penelitian');
		...
		if ($dtjnsoutput->num_rows() > 0) {
			$heads = array('No','Nama Syarat Seminar Hasil Penelitian','Keterangan','di Tujukan ke','Tipe Field','Status');
			...
			foreach ($dtjnsoutput->result() as $row) {
				...
				/*$rows[] = 	$row->kode_Syarat Seminar Hasil Penelitian;*/
				...
				if (!in_array($offset,array("cetak","excel"))) {
					//$ubah = anchor(site_url($this->dir.'/add_data/'.in_de(array('id_ref_seminar_hp'=>$row->id_ref_seminar_hp))), '<i class="fa fa-pencil"></i>', 'class="btn btn-xs btn-flat btn-warning" ');
					$ubah = anchor('#','<i class="fa fa-pencil"></i>', 'class="btn btn-xs btn-warning btn-edit btn-flat" act="'.site_url($this->dir.'/add_data/'.in_de(array('id_ref_seminar_hp'=>$row->id_ref_seminar_hp))).'" title="Edit Data Agenda Kegiatan..."');
					
					/*$ubah = anchor('#','<i class="fa fa-pencil"></i>','class="btn btn-xs btn-flat btn-warning btn-edit" act="'.site_url($this->dir.'/add_data/'.in_de(array('id_ref_seminar_hp'=>$row->id_ref_seminar_hp))).'" title="Klik untuk edit data"');*/
					$hapus = anchor('#','<i class="fa fa-trash"></i>','class="btn btn-xs btn-flat btn-danger btn-delete" act="'.site_url($this->dir.'/delete_data/'.in_de(array('id_ref_seminar_hp'=>$row->id_ref_seminar_hp))).'" msg="Apakah anda yakin ingin menghapus data ini ?" title="klik untuk menghapus data"');
					$rows[] = 	$ubah;
					$rows[] = 	$hapus;
				}
...
/*
		$btn_tambah = anchor('#','<i class="fa fa-plus fa-btn"></i>Nama Syarat Seminar Hasil Penelitian', 'class="btn btn-success btn-edit btn-flat" act="'.site_url($this->dir.'/add_data').'" title="Klik untuk tambah data"');*/
		//$btn_tambah = anchor(site_url($this->dir.'/add_data'), '<i class="fa fa-plus"></i> Nama Syarat Seminar Hasil Penelitian', 'class="btn btn-md btn-success btn-flat"');
		
		$btn_tambah = anchor('#','<i class="fa fa-plus fa-btn"></i> Tambah Syarat Seminar Hasil Penelitian', 'class="btn btn-success btn-edit btn-flat" act="'.site_url($this->dir.'/add_data/').'" title="Klik untuk tambah data"');
		...
		$title = 'Referensi Syarat Seminar Hasil Penelitian';
	...
	function on($par) {
		...
		$param1 =
			...
			$param1['where'] = array('id_ref_seminar_hp'=>$o['id_ref_seminar_hp']);
	...
	function urut($par) {
		...
		$param1 =
			...
			$param1['where'] = array('id_ref_seminar_hp'=>$o['id1']);
			$this->general_model->simpan_data($param1);
			/*die();*/
		...
		$param2 =
			...
			$param2['where'] = array('id_ref_seminar_hp'=>$o['id2']);
			 $this->general_model->simpan_data($param2);




		//$this->general_model->save_data('ref_seminar_hp',array('urut' => $o['no2']),'id_ref_seminar_hp',$o['id1']);
		//$this->general_model->save_data('ref_seminar_hp',array('urut' => $o['no1']),'id_ref_seminar_hp',$o['id2']);
		redirect($this->dir);
    ...
    public function add_data($param=NULL){
    	$o = un_de($param);
    	$id= $o['id_ref_seminar_hp'];
        $data = array(
            'button' => 'Tambah',
            'action' => site_url('tigris/ref_seminar_hp/save_aksi'),

        'id_ref_seminar_hp' => set_value('id_ref_seminar_hp'),
		);
        $from = array(
			'ref_seminar_hp a' => '',
			'ref_tipe_field b' => array('a.id_ref_tipe_field = b.id_ref_tipe_field','left')
		);
		$data['title'] = (!empty($code)) ? 'Ubah Data Syarat Seminar Hasil Penelitian' : 'Syarat Seminar Hasil Penelitian Baru';
       	$dt = !empty($id) ?  $this->general_model->datagrab(array(
					'tabel' => $from,
					'where' => array('a.id_ref_seminar_hp' => $id)))->row() : null;
		...
		$data['form_data'] = '';
		$data['form_data'] .= '<input type="hidden" name="id_ref_seminar_hp" class="id_ref_seminar_hp" value="'.$id .'"/>';
		$data['form_data'] .= '<div class="row">';
		$data['form_data'] .= '<div class="col-lg-6">';
			$data['form_data'] .= '<p><label>Nama Syarat Seminar Hasil Penelitian</label>';
			$data['form_data'] .= form_textarea('nama_syarat', @$dt->nama_syarat,'class="form-control" placeholder="Nama Syarat Seminar Hasil Penelitian" required');
			...
			$data['form_data'] .= form_input('keterangan_seminar_hp', @$dt->keterangan_seminar_hp,'class="form-control" placeholder="Nama Syarat Seminar Hasil Penelitian" required');
			$data['form_data'] .= '</div>';
		$data['form_data'] .= '</div>';
    ...
    function save_aksi(){
    	$id_ref_seminar_hp = $this->input->post('id_ref_seminar_hp');
    	...
    	$keterangan_seminar_hp = $this->input->post('keterangan_seminar_hp',TRUE);
    	/*$kode_Syarat Seminar Hasil Penelitian = $this->input->post('kode_Syarat Seminar Hasil Penelitian');*/
    	$tgl_skrg = date('Y-m-d');
     	$u = $this->general_model->datagrab(array(
            ...
            if(empty($id_ref_seminar_hp)) {
            	
            	$cek_prop = $this->general_model->datagrabs(array('tabel'=>'ref_seminar_hp','where'=>array('nama_syarat'=>$nama_syarat),'select'=>'id_ref_seminar_hp, MAX(id_ref_seminar_hp) as id'))->row();
                if(empty($cek_prop->id_ref_seminar_hp)) {
                	$id_prop = $this->general_model->save_data('ref_seminar_hp',array('nama_syarat' => $nama_syarat,'id_ref_tipe_field' => $id_ref_tipe_field,'id_bidang' => $id_bidang,'wajib_isi' => $wajib_isi,'keterangan_seminar_hp' => $keterangan_seminar_hp,'urut' => $urut,'status' =>1));
	                $this->session->set_flashdata('ok', 'Data Berhasil Disimpan...');
	            }else{
	                $id_prop = $cek_prop->id_ref_seminar_hp ;
	                $this->session->set_flashdata('fail', 'Nama Syarat Seminar Hasil Penelitian sudah ada...');
	            }
            }else{
            	...
            	$par = array(
					...
					'data'=>array(
						...
						'keterangan_seminar_hp' => $keterangan_seminar_hp,
						'wajib_isi'=>$wajib_isi
						),
					);

					$par['where'] = array('id_ref_seminar_hp'=>$id_ref_seminar_hp);
	...
	function delete_data($code) {
		$sn = un_de($code);
		$id_ref_seminar_hp = $sn['id_ref_seminar_hp'];
		$del = $this->general_model->delete_data('ref_seminar_hp','id_ref_seminar_hp',$id_ref_seminar_hp);
		if ($del) {
