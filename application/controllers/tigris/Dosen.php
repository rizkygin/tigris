...
class Dosen extends CI_Controller {
	...
	public function list_data($search=NULL, $offset=NULL) {
		$data['breadcrumb'] = array($this->dir => 'Referensi Dosen');
		...
		if (!empty($search_key)) {
			$fcari = array(
				'nama' 		=> $search_key,
				'nip' 		=> $search_key,
				'tipe_dosen' => $search_key
			);	
			$data['for_search'] = $fcari['nama'];
			$data['for_search'] = $fcari['nip'];
			$data['for_search'] = $fcari['tipe_dosen'];
		} else if ($search) {
			$fcari=un_de($search);
			$data['for_search'] = @$fcari['nama'];
			$data['for_search'] = @$fcari['nip'];
		...
		$from = array(
			...
			'ref_tipe_dosen d' => array('a.id_ref_tipe_dosen  = d.id_ref_tipe_dosen','left')
		);
		$where = array('a.id_tipe'=>2);
		$config['per_page']		= '10';
		...
		$config['base_url']	= site_url($this->dir.'/Dosen/list_data/'.in_de($fcari));
		...
		if ($dtjnsoutput->num_rows() > 0) {
			$heads = array('No','Nama','Nomor Induk Dosen','Status');
			...
			foreach ($dtjnsoutput->result() as $row) {
				...
				$link1 = '<a href="#" act="'.site_url($this->dir.'/Dosen/add_operator/'.$row->id_pegawai).'" class="btn btn-xs btn-warning btn-edit"><i class="fa fa-pencil"></i></a>';
				$link2= ' <a href="'.site_url($this->dir.'/Dosen/reset_operator/'.$row->id_pegawai).'" class="btn btn-xs btn-primary btn-reset" title="Reset Password" msg="Yakin untuk mereset password User? <br>(Password standar adalah <b>qwerty</b>)"><i class="fa fa-repeat"></i></a>';
				...
				if ($this->indie) {
					$rows[] = anchor($this->dir.'/Dosen/delete_data/'.$row->id_pegawai.'/'.(in_de($fcari).'~'.$offset),'
...
/*
		$btn_tambah = anchor('#','<i class="fa fa-plus fa-btn"></i>Nama Dosen', 'class="btn btn-success btn-edit btn-flat" act="'.site_url($this->dir.'/add_operator').'" title="Klik untuk tambah data"');*/
		//$btn_tambah = anchor(site_url($this->dir.'/add_operator'), '<i class="fa fa-plus"></i> Nama Dosen', 'class="btn btn-md btn-success btn-flat"');
		
		$btn_tambah = anchor('#','<i class="fa fa-plus fa-btn"></i> Tambah Dosen', 'class="btn btn-success btn-edit btn-flat" act="'.site_url($this->dir.'/Dosen/add_operator/').'" title="Klik untuk tambah data"');
		...
		$btn_cetak =
			...
			<li>'.anchor($this->dir.'/Dosen/list_data/'.in_de($fcari).'/cetak','<i class="fa fa-print"></i> Cetak','target="_blank"').'</li>
			<li>'.anchor($this->dir.'/Dosen/list_data/'.in_de($fcari).'/excel','<i class="fa fa-file-excel-o"></i> Ekspor Excel','target="_blank"').'</li>
			</ul>
			</div>';
		$data['extra_tombol'] = 
				form_open($this->dir.'/Dosen/list_data','id="form_search" role="form"').
	...
	function add_operator($id = null) {
		...
		$data['title']	= empty($id) ? 'Tambah Dosen' : 'Ubah Dosen';
		...
		$data['unit'] = $this->unit;
			...
			$data['link_unit'] = site_url($this->dir.'/Dosen/cb_bidang');
			$data['cb_unit'] = $this->general_model->combo_box(array(
			'tabel' => 'ref_unit','key' => 'id_unit','val' => array('unit'),'order' => 'urut'));

		$data['cb_tipe_dosen'] = $this->general_model->combo_box(array(
			'tabel' => 'ref_tipe_dosen','key' => 'id_ref_tipe_dosen','val' => array('tipe_dosen')));
		...
		$data['link_save'] = $this->dir.'/Dosen/save_user';
		$data['link_foto'] = $this->dir.'/Dosen/save_foto';
		$data['pegawai_role'] = $r;
		$this->load->view('umum/dosen_form',$data);
	...
	function save_user(){
		...
		$s = array(
			...
			'id_ref_tipe_dosen' => $this->input->post('id_ref_tipe_dosen'),
