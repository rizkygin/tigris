...
class Seminar_hp extends CI_Controller {
	...
	public function list_data($search=NULL, $offset=NULL) {
		...
		$dtjnsoutput = $this->general_model->datagrab(array('tabel'=>$from, 'order'=>'id_seminar_hp ASC', 'limit'=>$lim, 'offset'=>$offs, 'search'=>$fcari,'select'=>$select,'where'=>$where,'order'=>'a.id_seminar_hp DESC'));
		...
		if ($dtjnsoutput->num_rows() > 0) {
			...
			foreach ($dtjnsoutput->result() as $row) {
				...
				$rows[] = 	((($cek_jml_akademik==$cek_akademik) AND ($cek_jml_perustakaan==$cek_perustakaan) AND ($cek_jml_keuangan==$cek_keuangan) AND $cek_jml==$cek_jml2) ? (($cek_status_shp->status_shp != 1)? '<span class="blink_me">dalam proses</div>' : '<i class="fa fa-check" style="color:blue"></i> Selesai') : 'Belum di verifikasi semua bidang');
...
/*
		$btn_tambah = anchor('#','<i class="fa fa-plus fa-btn"></i>Nama Seminar Hasil Penelitian', 'class="btn btn-success btn-edit btn-flat" act="'.site_url($this->dir.'/add_data').'" title="Klik untuk tambah data"');*/
		//$btn_tambah = anchor(site_url($this->dir.'/add_data'), '<i class="fa fa-plus"></i> Nama Seminar Hasil Penelitian', 'class="btn btn-md btn-success btn-flat"');
		$id_pegawai = $this->session->userdata('id_pegawai');
		...
		$cek_peseminar_hp2 = $this->general_model->datagrab(array('tabel'=>'seminar_hp','where'=>array('id_mahasiswa'=>@$id_pegawai,'status_shp'=>NULL)))->num_rows();
		$cek_seminar_hpx = $this->general_model->datagrab(array('tabel'=>'seminar_hp','where'=>array('id_mahasiswa'=>@$id_pegawai)))->num_rows();
		...
		if($this->general_model->check_role($this->session->userdata('id_pegawai'),"mhs")){
			if($cek_tahun->nama_tahun < 2020){
				if(date('Y-m-d') >= $cek_tanggal->row('start_date') AND date('Y-m-d') <= $cek_tanggal->row('end_date')){
					if($cek_peseminar_hp2 == 1){

							$btn_tambah ='';
						}else{
							
							$btn_tambah= anchor(site_url($this->dir.'/tambah_data/'.$cek_tanggal->row('id_periode_pu').'/'.$cek_tanggal->row('id_ref_semester')),'<i class="fa fa-plus fa-btn"></i> Tambah Seminar HP', 'class="btn btn-success btn-editx btn-flat" act="" title="Klik untuk tambah data"');
					...
					if(date('Y-m-d') >= $cek_tanggal->row('start_date') AND date('Y-m-d') <= $cek_tanggal->row('end_date')){
						...
						if($cek_peseminar_hp2 == 1){
							...
							if($cek_pengajuan_judul3 == 1){
								$btn_tambah = anchor(site_url($this->dir.'/pendaftaran_ujian/'.$cek_tanggal->row('id_periode_pu').'/'.$cek_tanggal->row('id_ref_semester')),'<i class="fa fa-plus fa-btn"></i> Tambah Seminar Hasil Penelitian', 'class="btn btn-success btn-editx btn-flat" act="" title="Klik untuk tambah data"');
							}else{
			...
			/*$btn_tambah = anchor('#','<i class="fa fa-plus fa-btn"></i> Tambah Seminar Hasil Penelitian', 'class="btn btn-success btn-edit btn-flat" act="'.site_url($this->dir.'/add_data/').'" title="Klik untuk tambah data"');*/
				...
				if($this->general_model->check_role($this->session->userdata('id_pegawai'),"mhs") AND date('Y-m-d') >= $cek_tanggal->row('start_date') AND date('Y-m-d') <= $cek_tanggal->row('end_date')){
					...
					if($this->general_model->check_role($this->session->userdata('id_pegawai'),"mhs") AND $cek_peseminar_hp2 == 1){
    ...
    public function add_data($param=NULL,$id_ref_semester=NULL){
		...
		$data['form_data'] .= '<div class="col-lg-12">';
			...
			$data['form_data'] .= '<br><br><h1>Syarat Seminar Hasil Penelitian</h1><hr>';
...
';
			...
			foreach ($dt_kom->result() as $kom) {
				...
				$dt_veri = $this->general_model->datagrab(array('tabel'=>'veri_seminar_hp','where'=>array('id_seminar_hp'=>$dt->id_seminar_hp, 'id_ref_seminar_hp'=>$kom->id_ref_seminar_hp, 'id_mahasiswa'=>$dt->id_mahasiswa)));
				$hapus_field = anchor('#',' x ','class="btn btn-xs btn-danger btn-delete" act="'.site_url($this->dir.'/delete_field/'.in_de(array('id_veri_seminar_hp'=>$dt_kom_pro->row('id_veri_seminar_hp'),'id_mahasiswa'=>$dt->id_mahasiswa,'id_seminar_hp'=>$dt->id_seminar_hp,'id_mhs_seminar_hp'=>$dtnilai->row('id_mhs_seminar_hp')))).'" msg="Apakah anda yakin ingin menghapus data ini ?" title="klik untuk menghapus data"');
...
*/
						...
						if($kom->id_ref_tipe_field == 1){
							...
							if($dtnilai->row('link') == NULL){
	          					...
	          					$isi_file = form_input('link['.$kom->id_ref_seminar_hp.']',($dtnilai->num_rows()==NULL) ? NULL : $dtnilai->row('link'),'class="form-control" placeholder="link" style="width:100%"');
		          			}else{
...
cek($id_unit);*/
		        ...
		        <tr>
		          ...
		          <div class="col-lg-12">'.form_hidden('id_berkas[]', $kom->id_ref_seminar_hp).$isi_file.'</th>
		          <th style="text-align:left">
    ...
    public function verifikasi($param=NULL){
		...
		if ($this->general_model->check_role($this->session->userdata('id_pegawai'),"pimp")) {
			...
			$data['form_data'] .= '<div class="col-lg-12">';
			$chk_pem = NULL;
				$dt_mhs = $this->general_model->datagrab(array('tabel'=>'mhs_penguji', 'where'=>array('id_pendaftaran_ujian'=>$dt->id_seminar_hp, 'id_pembimbing'=>$kom->id_pegawai, 'id_mahasiswa'=>$dt->id_mahasiswa, 'tipe_ujian'=>2)));
			...
			//cek($cek_jumlah_pem->x);
				...
				if(($cek_jumlah_pem->x+$cek_jumlah_shp->x+$cek_jumlah_t->x) >= 6){
					...
					$data['form_data'] .= '<input  name="pemb[]'.$kom->id_pegawai.'" '.$chk_pem.' type="checkbox" value="'.$kom->id_pegawai.'" class="incheck" style="margin-top: -2px"> '.$kom->nama.'---'.$kom->tipe_dosen.'---'.' (<b>'.($cek_jumlah_pem->x+$cek_jumlah_shp->x+$cek_jumlah_t->x).'</b>)';
				}
		...
		$data['form_data'] .= '<div class="col-lg-12">';
			...
			$dt_sem = $this->general_model->datagrab(array('tabel'=>'seminar_hp','where'=>array('id_seminar_hp'=>$dt->id_seminar_hp)))->row();
...
$no = 1;
			foreach ($dt_kom->result() as $kom) {
				...
				/*cek($kom->wajib_isi);
				die();*/
				$hapus_field = anchor('#',' x ','class="btn btn-xs btn-danger btn-delete" act="'.site_url($this->dir.'/delete_field_ver/'.in_de(array('id_veri_seminar_hp'=>$dt_kom_pro->row('id_veri_seminar_hp'),'id_mahasiswa'=>$dt->id_mahasiswa,'id_seminar_hp'=>$dt->id_seminar_hp,'id_mhs_seminar_hp'=>$dtnilai->row('id_mhs_seminar_hp')))).'" msg="Apakah anda yakin ingin menghapus data ini ?" title="klik untuk menghapus data ini"');
				$hapus_verifikasi = anchor('#',' x ','class="btn btn-xs btn-warning btn-delete" act="'.site_url($this->dir.'/delete_verifikasi/'.in_de(array('id_veri_seminar_hp'=>$dt_kom_pro->row('id_veri_seminar_hp'),'id_mahasiswa'=>$dt->id_mahasiswa,'id_seminar_hp'=>$dt->id_seminar_hp,'id_mhs_seminar_hp'=>$dtnilai->row('id_mhs_seminar_hp')))).'" msg="Apakah anda yakin ingin menghapus data ini ?" title="klik untuk merubah status verifikasi"');
...
*/
			...
			/*$data['form_data'] .=  form_radio('status_ver['.$kom->id_ref_seminar_hp.']',($dtnilai->num_rows()==NULL) ? NULL : $dtnilai->row('status_ver'),'class="form-control" placeholder="status_ver" style="width:100%"');*/
			...
			if($dt_kom_pro->row('status_ver') == NULL){
				...
				if($dtnilai->row('berkas') != NULL OR $kom->id_ref_tipe_field == 1 OR $dtnilai->row('link') != NULL){
					...
					$data['form_data'] .=  '<label>
		                      <input type="radio" value="1" name="status_ver['.$kom->id_ref_seminar_hp.']" class="flat-blue" '.((!empty($dt_kom_pro) and $dt_kom_pro->row('status_ver') == "1") ? 'checked' : '').' /> <i>Lulus</i> 
		                    </label>&nbsp;  &nbsp;  &nbsp;';
					$data['form_data'] .=  '<label>
		                      <input type="radio" value="2" name="status_ver['.$kom->id_ref_seminar_hp.']" class="flat-blue" '.((!empty($dt_kom_pro) and $dt_kom_pro->row('status_ver') == "2") ? 'checked' : '').' /> <i>Tolak</i> 
		                    </label>&nbsp;  &nbsp;  &nbsp;';
					$data['form_data'] .=  form_input('catatan['.$kom->id_ref_seminar_hp.']',($dtnilai->num_rows()==NULL) ? NULL : $dtnilai->row('catatan'),'class="form-control" placeholder="catatan" style="width:100%"');
...
if ($this->general_model->check_role($this->session->userdata('id_pegawai'),"pimp")) {
			...
			$data['form_data'] .= '<p><label>Status Seminar Hasil Penelitian</label><p>';
			$data['form_data'] .= '<label>
                      <input type="radio" value="1" name="status_n_shp" class="flat-blue" '.((!empty($dt) and $dt->status_n_shp == "1") ? 'checked' : '').' /> <i>Lulus</i> 
                    </label>&nbsp;  &nbsp;  &nbsp;';
			$data['form_data'] .= '<label>
                      <input type="radio" value="2" name="status_n_shp" class="flat-blue" '.((!empty($dt) and $dt->status_n_shp == "2") ? 'checked' : '').' /> <i>Berhenti/ Mengulang</i>
                    </label>';
}
			$data['form_data'] .= '</div>';
		$data['form_data'] .= '</div>';
    	...
    	$tgl_skrg = date('Y-m-d');
            if(empty($id_seminar_hp)) {
                ...
                if(empty($cek_prop->id_seminar_hp)) {
                	...
                	$id_prop = $this->general_model->save_data('seminar_hp',array('id_mahasiswa' => $id_pegawai,'judul_tesis' => $judul_tesis,'id_ref_semester' => $id_ref_semester,'id_periode_pu' => $id_periode_pu,'tgl_pj' => date('Y-m-d')));
			...
			foreach ($berkas as $key => $value) {
				...
				$cek_linkx = $this->general_model->datagrabs(array('tabel'=>'veri_seminar_hp','where'=>array('id_seminar_hp'=>$id_seminar_hp,'id_mahasiswa'=>$id_mahasiswa,'id_ref_seminar_hp'=>$value)))->row();
				if(@$cek_linkx->id_veri_seminar_hp !=NULL){
					$cek_linkxx = $this->general_model->datagrabs(array('tabel'=>'veri_seminar_hp','where'=>array('id_seminar_hp'=>$cek_linkx->id_seminar_hp,'id_mahasiswa'=>$cek_linkx->id_mahasiswa,'id_ref_seminar_hp'=>$cek_linkx->id_ref_seminar_hp)))->num_rows();
									/*cek($value);
									die();*/
									if($cek_linkxx > 0) 
										...
										'where' => array(
											'id_seminar_hp' => $id_seminar_hp,'id_mahasiswa' => $id_mahasiswa,'id_ref_seminar_hp' => $value)));
									}
				}
			...
			foreach ($link as $key => $value) {
				$pars = array(
					'tabel'=>'mhs_seminar_hp',
					'data'=>array(
						'id_seminar_hp'=>$id_seminar_hp,
						'id_mahasiswa' => $id_mahasiswa,
						'id_ref_seminar_hp' => $key,
						'link' => $value
						),
					);

				$cek_link = $this->general_model->datagrabs(array('tabel'=>'mhs_seminar_hp','where'=>array('id_seminar_hp'=>$id_seminar_hp,'id_mahasiswa'=>$id_mahasiswa,'id_ref_seminar_hp'=>$key)))->num_rows();
				/*cek($cek_link);
				die();*/
				if($cek_link > 0) $pars['where'] = array('id_seminar_hp'=>$id_seminar_hp,'id_mahasiswa'=>$id_mahasiswa,'id_ref_seminar_hp'=>$key);
...
/*
				...
				/*cek($key);
				die();*/
				$cek_linkx = $this->general_model->datagrabs(array('tabel'=>'veri_seminar_hp','where'=>array('id_seminar_hp'=>$id_seminar_hp,'id_mahasiswa'=>$id_mahasiswa,'id_ref_seminar_hp'=>$key)))->num_rows();
				
				if($cek_linkx > 0) 
					$this->general_model->delete_data(array(
					'tabel' => 'veri_seminar_hp',
					'where' => array(
						'id_seminar_hp' => $id_seminar_hp,'id_mahasiswa' => $id_mahasiswa,'id_ref_seminar_hp' => $key)));



				/*if($cek_linkx > 0) $this->general_model->delete_data('veri_seminar_hp','id_seminar_hp',$id_seminar_hp,'id_mahasiswa',$id_mahasiswa,'id_ref_seminar_hp',$key);
*/
            ...
            redirect($this->dir.'/add_data/'.in_de(array('id_mahasiswa'=>$id_mahasiswa,'id_seminar_hp'=>$id_seminar_hp)));
}
...
*//*cek($status_ver);
		...
		$status_n_shp = $this->input->post('status_n_shp');
		if($status_n_shp == NULL){
			$status_n_shp = 1;
		}else{
			$status_n_shp = $status_n_shp;
		}
		...
		$id_seminar_hp = $this->input->post('id_pendaftaran_ujian');
    	if($this->general_model->check_role($this->session->userdata('id_pegawai'),"pimp")){
			...
			$this->general_model->delete_data(array(
					'tabel' => 'mhs_penguji',
					'where' => array(
						'id_pendaftaran_ujian' => $id_seminar_hp,'id_mahasiswa' => $id_mahasiswa,'id_ref_semester' => $id_ref_semester,'id_periode_pu' => $id_periode_pu,
									'tipe_ujian' =>2)));
				foreach ($pemb as $key => $value) {
					$this->general_model->save_data('mhs_penguji', array(
						'id_pendaftaran_ujian'=>$id_seminar_hp,
						'tipe_ujian'=>2,
						'id_mahasiswa'=>$id_mahasiswa,
						'id_ref_semester'=>$id_ref_semester,
						'id_periode_pu'=>$id_periode_pu,
			...
			$par = array(
				...
				$cek_judul = $this->general_model->datagrabs(array('tabel'=>'seminar_hp','where'=>array('id_seminar_hp'=>$id_seminar_hp)))->row();
...
die();*/
				if($status_n_shp == 2){
							...
							$parxx['where'] = array(
								'id_pendaftaran_ujian' => $id_seminar_hp,'id_mahasiswa' => $id_mahasiswa,'id_ref_semester' => $id_ref_semester,'id_periode_pu' => $id_periode_pu,'tipe_ujian' =>2);
		...
		/*if($link != NULL){*/
			...
			foreach (@$link as $key => $value) {
				...
				$cek_link = $this->general_model->datagrabs(array('tabel'=>'mhs_seminar_hp','where'=>array('id_seminar_hp'=>$id_seminar_hp,'id_mahasiswa'=>$id_mahasiswa,'id_ref_seminar_hp'=>$key)));
				/*cek($cek_link);
				die();*/
				if($cek_link->num_rows() > 0) $pars['where'] = array('id_seminar_hp'=>$id_seminar_hp,'id_mahasiswa'=>$id_mahasiswa,'id_ref_seminar_hp'=>$key);
			...
			foreach ($status_ver as $key => $value) {
				...
				$cek_linkx = $this->general_model->datagrabs(array('tabel'=>'veri_seminar_hp','where'=>array('id_seminar_hp'=>$id_seminar_hp,'id_mahasiswa'=>$id_mahasiswa,'id_ref_seminar_hp'=>$key,'status_ver'=>2)))->row();
				if(@$cek_linkx->id_veri_seminar_hp != NULL){
					$cek_linkxx = $this->general_model->datagrabs(array('tabel'=>'veri_seminar_hp','where'=>array('id_seminar_hp'=>$cek_linkx->id_seminar_hp,'id_mahasiswa'=>$cek_linkx->id_mahasiswa,'id_ref_seminar_hp'=>$cek_linkx->id_ref_seminar_hp)))->num_rows();
					/*cek($cek_linkx);
					die();*/
			...
			foreach ($catatan as $key => $value) {
				$parx = array(
					...
					$parx['where'] = array('id_seminar_hp'=>$id_seminar_hp,'id_mahasiswa'=>$id_mahasiswa,'id_ref_seminar_hp'=>$key);
		...
		redirect($this->dir.'/verifikasi/'.in_de(array('id_mahasiswa'=>$id_mahasiswa,'id_seminar_hp'=>$id_seminar_hp)));
	...
	function delete_field($code) {
		$sn = un_de($code);
		$id_veri_seminar_hp = $sn['id_veri_seminar_hp'];
		$id_mhs_seminar_hp = $sn['id_mhs_seminar_hp'];
		$id_mahasiswa = $sn['id_mahasiswa'];
		$id_seminar_hp = $sn['id_seminar_hp'];
		/*cek($id_mhs_seminar_hp);
		cek($id_mahasiswa);
		cek($id_seminar_hp);
		die();*/
		$del = $this->general_model->delete_data('mhs_seminar_hp','id_mhs_seminar_hp',$id_mhs_seminar_hp);


		$del = $this->general_model->delete_data('veri_seminar_hp','id_veri_seminar_hp',$id_veri_seminar_hp);
		$delx = $this->general_model->delete_data('mhs_seminar_hp','id_mhs_seminar_hp',$id_mhs_seminar_hp);
    ...
    function save_aksi_judul(){
    	...
    	$id_seminar_hp = $this->input->post('id_seminar_hp');
    	$id_mahasiswa = $this->input->post('id_mahasiswa');
/*cek($id_seminar_hp);cek($judul_tesis);die();
cek($id_ref_semester);
cek($id_periode_pu);
cek($id_mahasiswa);
*/
		...
		$pemb = $this->input->post('pemb');
    	 if(empty($id_seminar_hp)) {
                ...
                if(empty($cek_prop->id_seminar_hp)) {
                	...
                	$id_prop = $this->general_model->save_data('seminar_hp',array('id_mahasiswa' => $id_pegawai,'judul_tesis' => $judul_tesis,'id_ref_semester' => $id_ref_semester,'id_periode_pu' => $id_periode_pu,'tgl_shp' => date('Y-m-d')));
	                

                	$id_seminar_hp = $this->db->insert_id();

                	$this->general_model->delete_data(array(
					'tabel' => 'mhs_pembimbing',
					'where' => array('id_mahasiswa' => $id_mahasiswa,'status_pemb' =>1)));
					/*'where' => array(
						'id_pengajuan_judul' => $id_seminar_hp,'id_mahasiswa' => $id_mahasiswa,'id_ref_semester' => $id_ref_semester,'id_periode_pu' => $id_periode_pu,'tipe'=>2)));*/
				foreach ($pemb as $key => $value) {
					$this->general_model->save_data('mhs_pembimbing', array(
						'id_pengajuan_judul'=>$id_seminar_hp,
						'id_mahasiswa'=>$id_mahasiswa,
						'id_ref_semester'=>$id_ref_semester,
						'id_periode_pu'=>$id_periode_pu,
						'id_pembimbing'=>$value,
            	...
            	$parx = array(
					...
					$parx['where'] = array('id_seminar_hp'=>$id_seminar_hp);
