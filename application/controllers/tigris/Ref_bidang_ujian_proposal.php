...
class Ref_bidang_ujian_proposal extends CI_Controller{
    var $dir = 'tigris/Ref_bidang_ujian_proposal';
    ...
    public function list_data($search=NULL, $offset=NULL) {
		$data['breadcrumb'] = array($this->dir => 'Bidang untuk Pengajuan Judul');
		...
		foreach ($nav->result() as $rowx) {
			$awal[] = array($rowx->id_bidang_ujian_proposal,$rowx->urut);
		}
		...
		if ($dtjnsoutput->num_rows() > 0) {
			...
			foreach ($dtjnsoutput->result() as $row) {
				$rows = array();
				if($row->status == 1){
					$status = anchor('tigris/Ref_bidang_ujian_proposal/on/'.in_de(array('id_bidang_ujian_proposal' => $row->id_bidang_ujian_proposal,'status' =>0)),'<i class="fa fa-fw fa-toggle-on text-aqua" style="font-size:20px;"></i>');
				}else{
					$status = anchor('tigris/Ref_bidang_ujian_proposal/on/'.in_de(array('id_bidang_ujian_proposal' => $row->id_bidang_ujian_proposal,'status' =>1)),'<i class="fa fa-fw fa-toggle-off text-default" style="font-size:20px;"></i>');

				}
				//cek($row->id_ref_bidang_ujian_proposal);
				$btn_down = ($m+1 < $nav->num_rows()) ? anchor('tigris/Ref_bidang_ujian_proposal/urut/'.in_de(array('id1' => $row->id_bidang_ujian_proposal,'no1' => $row->urut,'id2' => $awal[$m+1][0],'no2' =>  $awal[$m+1][1])),'<i class="fa fa-arrow-down"></i>') : ' &nbsp; ';
					$btn_up = ($m > 0) ? anchor('tigris/Ref_bidang_ujian_proposal/urut/'.in_de(array('id2' => $row->id_bidang_ujian_proposal,'no2' => $row->urut,'id1' =>  !empty($urutkan[0])?$urutkan[0]:@$awal[0],'no1' => !empty($urutkan[1])?$urutkan[1]:@$awal[1])),'<i class="fa fa-arrow-up"></i>') : ' &nbsp; ';
					$urutkan = array($row->id_bidang_ujian_proposal,$row->urut);
				...
				//simpan urutan 
				$this->general_model->save_data('ref_bidang_ujian_proposal',[
					'urut' => $no
				],
				'id_bidang_ujian_proposal', $row->id_bidang_ujian_proposal
				);
				...
				if (!in_array($offset,array("cetak","excel"))) {
					...
					// $ubah = anchor('#','<i class="fa fa-pencil"></i>', 'class="btn btn-xs btn-warning btn-edit btn-flat" act="'.site_url($this->dir.'/add_data/'.in_de(array('id_bidang_ujian_proposal'=>$row->id_bidang_ujian_proposal))).'" title="Edit Data Agenda Kegiatan..."');
					
					/*$ubah = anchor('#','<i class="fa fa-pencil"></i>','class="btn btn-xs btn-flat btn-warning btn-edit" act="'.site_url($this->dir.'/add_data/'.in_de(array('id_bidang_ujian_proposal'=>$row->id_bidang_ujian_proposal))).'" title="Klik untuk edit data"');*/
					$hapus = anchor('#','<i class="fa fa-trash"></i>','class="btn btn-xs btn-flat btn-danger btn-delete" act="'.site_url($this->dir.'/delete_data/'.in_de(array('id_bidang_ujian_proposal'=>$row->id_bidang_ujian_proposal))).'" msg="Apakah anda yakin ingin menghapus data ini ?" title="klik untuk menghapus data"');
					// $rows[] = 	$ubah;
					$rows[] = 	$hapus;
				}
		...
		$title = 'Referensi Urutan Bidang Ujian Proposal';
    ...
    public function add_data($param=NULL){
    	$o = un_de($param);
    	$id= @$o['id_bidang_ujian_proposal'];
        $data = array(
            'button' => 'Tambah',
            'action' => site_url('tigris/Ref_bidang_ujian_proposal/save_aksi'),

        'id_bidang_ujian_proposal' => set_value('id_bidang_ujian_proposal'),
		);
        $from = array(
			'ref_bidang_ujian_proposal a' => ''
		);
		...
		$data['form_data'] = '';
		$data['form_data'] .= '<input type="hidden" name="id_bidang_ujian_proposal" class="id_bidang_ujian_proposal" value="'.$id .'"/>';
    ...
    function delete_data($code) {
		$sn = un_de($code);
		$id_bidang_ujian_proposal = $sn['id_bidang_ujian_proposal'];
		$del = $this->general_model->delete_data('ref_bidang_ujian_proposal','id_bidang_ujian_proposal',$id_bidang_ujian_proposal);
		if ($del) {
    ...
    function save_aksi(){
    	...
    	$id_bidang_ujian_proposal = $this->input->post('id_bidang_ujian_proposal');
    	...
    	$tgl_skrg = date('Y-m-d');
     	$u = $this->general_model->datagrab(array(
            ...
            if(empty($id_bidang_ujian_proposal)) {
                ...
                if(empty($cek_prop->id_bidang)) {
                	$id_prop = $this->general_model->save_data('ref_bidang_ujian_proposal',array('id_ref_bidang' => $input['id_bidang'],'urut' => $urut));	
            	...
            	$par = array(
					...
					$par['where'] = array('id_bidang_ujian_proposal'=>$id_bidang_ujian_proposal);
	...
	function urut($par) {
		...
		$param1 =
			...
			$param1['where'] = array('id_bidang_ujian_proposal'=>$o['id1']);
		...
		$param2 =
			...
			$param2['where'] = array('id_bidang_ujian_proposal'=>$o['id2']);
	...
	function on($par) {
		...
		$param1 =
			...
			$param1['where'] = array('id_bidang_ujian_proposal'=>$o['id_bidang_ujian_proposal']);
			$this->general_model->simpan_data($param1);
			redirect($this->dir);
	}
}
