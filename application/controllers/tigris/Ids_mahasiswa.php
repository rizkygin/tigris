...
class Ids_mahasiswa extends CI_Controller {
	...
	function index($offset=NULL) {
        ...
        $config_news['base_url']    = base_url().'tigris/Ids_mahasiswa/ajaxPengumuman';
        ...
        $totalRec = $this->general_model->datagrab(array(
			'tabel' => $from_pj, 
					'select'=>$select,
					'order'=>'a.id_pengajuan_judul DESC',
					'where'=>array('a.status_pj'=>null),
					/*'where'=>array('f.tahun = YEAR(NOW())'=>null),*/
					'offset'=>$offset
			))->num_rows();

        $config['target']      = '#postPST';
        $config['base_url']    = base_url().'tigris/Ids_mahasiswa/ajaxPaginationData';
		...
		if ($detail_kegiatan->num_rows() > 0) {
			...
			$heads3[] = array('data' => 'Mahasiswa','style'=>'background-color: #e8e8e8;width:20%');
			$heads3[] = array('data' => 'NIM','style'=>'background-color: #e8e8e8;width:20%');
			$heads3[] = array('data' => 'Tahun','style'=>'background-color: #e8e8e8;');
			$heads3[] = array('data' => 'Tanggal Pendaftaran','style'=>'background-color: #e8e8e8;');
			$heads3[] = array('data' => 'Progres','style'=>'background-color: #e8e8e8;');
			...
			foreach ($detail_kegiatan->result() as $row) {
				...
				$rows[] = 	array('data'=>$row->nip,'style'=>''.@$bg.'');
				$rows[] = 	array('data'=>$row->nama_tahun,'style'=>''.@$bg.'');
				$rows[] = 	array('data'=>$row->tgl_pj,'style'=>''.@$bg.'');
				//cek(date('d'));
				//cek(substr($row->tgl_mulai,8,2));
				
				$rows[] = 	array('data'=>'','style'=>''.@$bg.'');
				$rows[] = 	array('data'=>'','style'=>''.@$bg.'');
        ...
        $config_gali['base_url']    = base_url().'tigris/Ids_mahasiswa/ajaxPaginationGAL3';
        ...
        $config_gal4['base_url']    = base_url().'tigris/Ids_mahasiswa/ajaxPaginationGAL4';
        ...
        $config_gal['base_url']    = base_url().'tigris/Ids_mahasiswa/ajaxPaginationGAL1';
...
// ./ GALERI FOTO 1
        ...
        $config_gal2['base_url']    = base_url().'tigris/Ids_mahasiswa/ajaxPaginationGAL2';
...
*/
		/*if($status1->id_theme == 1){
			$this->load->view('tigris/kolom_mahasiswa',$data);
		}else{
	...
	function ajaxPaginationData()
    {
		...
		if ($detail_kegiatan->num_rows() > 0) {
			...
			foreach ($detail_kegiatan->result() as $row) {
				...
				$awal  = new DateTime($row->tgl_pj);
				$akhir = new DateTime(); // Waktu sekarang
				$diff  = $awal->diff($akhir);
//die();
				/*cek($awal);
				cek($akhir);
				cek(date_diff($awal,$akhir));
				die();*/
				/*
				@$selisih = @$awal-@$akhir;

				$diff  = date_diff( $awal, $akhir );cek($row->tgl_pj);
				cek(date('Y-m-d'));
				$tampil_date = date_diff($row->tgl_pj,date('Y-m-d'));*/
				/*cek($tampil_date);die();*/
				
				$seislih = ''.@$diff->y. ' tahun, '.@$diff->m . ' bulan, '.@$diff->d . ' hari';
				$rows[] = 	array('data'=>$seislih,'style'=>''.@$bg.'');
				$rows[] = 	array('data'=>'','style'=>''.@$bg.'');
				$rows[] = 	array('data'=>'','style'=>''.@$bg.'');
	...
	function ajaxPaginationGAL3()
    {
        ...
        $config_gal3['base_url']    = base_url().'tigris/Ids_mahasiswa/ajaxPaginationGAL3';
	...
	function ajaxPaginationGAL2(){
        ...
        $config_gal['base_url']    = base_url().'tigris/Ids_mahasiswa/ajaxPaginationGAL2';
        $config_gal['total_rows']  = $totalRec_gal;
        $config_gal['per_page']    = 1;
       	$config_gal['uri_segment'] = '4';
		...
		if ($st_data == 1) { 
			$data .='
				...
				function() {
					...
					$.ajax({
						url: \''.site_url("tigris/Ids_mahasiswa/get_durasi/").'/\'+ lf,
						method: \'GET\',
						dataType:"JSON",
