...
class Ref_pengajuan_judul extends CI_Controller {
	var $dir = 'tigris/Ref_pengajuan_judul';
	...
	public function list_data($search=NULL, $offset=NULL) {
		$data['breadcrumb'] = array($this->dir => 'Referensi Syarat Pengajuan Judul');
		...
		if ($dtjnsoutput->num_rows() > 0) {
			$heads = array('No','Nama Syarat Pengajuan Judul','Keterangan','di Tujukan ke','Tipe Field','Status');
			...
			foreach ($dtjnsoutput->result() as $row) {
				...
				if (!in_array($offset,array("cetak","excel"))) {
					...
					$ubah = anchor('#','<i class="fa fa-pencil"></i>', 'class="btn btn-xs btn-warning btn-edit btn-flat" act="'.site_url($this->dir.'/add_data/'.in_de(array('id_ref_pengajuan_judul'=>$row->id_ref_pengajuan_judul))).'" title="Edit Data Agenda Kegiatan..."');
					
					/*$ubah = anchor('#','<i class="fa fa-pencil"></i>','class="btn btn-xs btn-flat btn-warning btn-edit" act="'.site_url($this->dir.'/add_data/'.in_de(array('id_ref_pengajuan_judul'=>$row->id_ref_pengajuan_judul))).'" title="Klik untuk edit data"');*/
					$hapus = anchor('#','<i class="fa fa-trash"></i>','class="btn btn-xs btn-flat btn-danger btn-delete" act="'.site_url($this->dir.'/delete_data/'.in_de(array('id_ref_pengajuan_judul'=>$row->id_ref_pengajuan_judul))).'" msg="Apakah anda yakin ingin menghapus data ini ?" title="klik untuk menghapus data"');
					$rows[] = 	$ubah;
					$rows[] = 	$hapus;
				}
		...
		$btn_tambah = anchor('#','<i class="fa fa-plus fa-btn"></i> Tambah Syarat Pengajuan Judul', 'class="btn btn-success btn-edit btn-flat" act="'.site_url($this->dir.'/add_data/').'" title="Klik untuk tambah data"');
		...
		$title = 'Referensi Syarat Pengajuan Judul';
	...
	function on($par) {
		...
		$param1 =
			...
			$param1['where'] = array('id_ref_pengajuan_judul'=>$o['id_ref_pengajuan_judul']);
	...
	function urut($par) {
		...
		$param1 =
			...
			$param1['where'] = array('id_ref_pengajuan_judul'=>$o['id1']);
			$this->general_model->simpan_data($param1);
			/*die();*/
		...
		$param2 =
			...
			$param2['where'] = array('id_ref_pengajuan_judul'=>$o['id2']);
    ...
    public function add_data($param=NULL){
    	$o = un_de($param);
    	$id= @$o['id_ref_pengajuan_judul'];
        $data = array(
            'button' => 'Tambah',
            'action' => site_url('tigris/ref_pengajuan_judul/save_aksi'),

        'id_ref_pengajuan_judul' => set_value('id_ref_pengajuan_judul'),
		);
        $from = array(
			'ref_pengajuan_judul a' => '',
			'ref_tipe_field b' => array('a.id_ref_tipe_field = b.id_ref_tipe_field','left')
		);
		$data['title'] = (!empty($code)) ? 'Ubah Data Syarat Pengajuan Judul' : 'Syarat Pengajuan Judul Baru';
       	$dt = !empty($id) ?  $this->general_model->datagrab(array(
					'tabel' => $from,
					'where' => array('a.id_ref_pengajuan_judul' => $id)))->row() : null;
		...
		$chk = NULL;
		$chk = (@$dt->wajib_isi_mhs != NULL) ? 'checked' : '';
		$data['form_link'] = $this->dir.'/save_aksi/'.$id;
		$data['form_data'] = '';
		$data['form_data'] .= '<input type="hidden" name="id_ref_pengajuan_judul" class="id_ref_pengajuan_judul" value="'.$id .'"/>';
		$data['form_data'] .= '<div class="row">';
		$data['form_data'] .= '<div class="col-lg-6">';
			$data['form_data'] .= '<p><label>Nama Syarat Pengajuan Judul</label>';
			$data['form_data'] .= form_textarea('nama_syarat', @$dt->nama_syarat,'class="form-control" placeholder="Nama Syarat Pengajuan Judul" required');
			...
			$data['form_data'] .= '<p>';
			$data['form_data'] .= '<input  name="wajib_isi_mhs" '.$chk.' type="checkbox" value="1" class="incheck" style="margin-top: -2px"> &nbsp; &nbsp;<label>*Wajib diisi Mahasiswa</label>';
			...
			$data['form_data'] .= form_input('keterangan_pengajuan_judul', @$dt->keterangan_pengajuan_judul,'class="form-control" placeholder="Keterangan" required');
			$data['form_data'] .= '</div>';
		$data['form_data'] .= '</div>';
    ...
    function save_aksi(){
    	$id_ref_pengajuan_judul = $this->input->post('id_ref_pengajuan_judul');
    	$id_bidang = $this->input->post('id_bidang');
    	$wajib_isi_mhs = $this->input->post('wajib_isi_mhs');
    	if($wajib_isi_mhs != NULL){
    		$wajib_isi_mhs = $this->input->post('wajib_isi_mhs');
    	}else{
    		$wajib_isi_mhs = NULL;
    	}
    	...
    	$keterangan_pengajuan_judul = $this->input->post('keterangan_pengajuan_judul',TRUE);
    	/*$kode_Syarat Pengajuan Judul = $this->input->post('kode_Syarat Pengajuan Judul');*/
    	$tgl_skrg = date('Y-m-d');
     	$u = $this->general_model->datagrab(array(
            ...
            if(empty($id_ref_pengajuan_judul)) {
            	
            	$cek_prop = $this->general_model->datagrabs(array('tabel'=>'ref_pengajuan_judul','where'=>array('nama_syarat'=>$nama_syarat),'select'=>'id_ref_pengajuan_judul, MAX(id_ref_pengajuan_judul) as id'))->row();
                if(empty($cek_prop->id_ref_pengajuan_judul)) {
                	$id_prop = $this->general_model->save_data('ref_pengajuan_judul',array('nama_syarat' => $nama_syarat,'id_ref_tipe_field' => $id_ref_tipe_field,'keterangan_pengajuan_judul' => $keterangan_pengajuan_judul,'id_bidang' => $id_bidang,'wajib_isi_mhs' => $wajib_isi_mhs,'urut' => $urut,'status' =>1));
	                $this->session->set_flashdata('ok', 'Data Berhasil Disimpan...');
	            }else{
	                $id_prop = $cek_prop->id_ref_pengajuan_judul ;
	                $this->session->set_flashdata('fail', 'Nama Syarat Pengajuan Judul sudah ada...');
	            }
            }else{
            	...
            	$par = array(
					...
					'data'=>array(
						...
						'keterangan_pengajuan_judul' => $keterangan_pengajuan_judul,
						'wajib_isi_mhs'=>$wajib_isi_mhs,/*,
						'urut' => $urut*/
						),
					);

					$par['where'] = array('id_ref_pengajuan_judul'=>$id_ref_pengajuan_judul);
	...
	function delete_data($code) {
		$sn = un_de($code);
		$id_ref_pengajuan_judul = $sn['id_ref_pengajuan_judul'];
		$del = $this->general_model->delete_data('ref_pengajuan_judul','id_ref_pengajuan_judul',$id_ref_pengajuan_judul);
		if ($del) {
