...
class Tesis extends CI_Controller {
	...
	public function list_data($search=NULL, $offset=NULL) {
		...
		$dtjnsoutput = $this->general_model->datagrabs(array('tabel'=>$from, 'order'=>'id_tesis ASC', 'limit'=>$lim, 'offset'=>$offs, 'search'=>$fcari,'where'=>$where));
		...
		$cek_proposal_tesis = $this->general_model->datagrab(array('tabel'=>'seminar_hp','where'=>array('id_mahasiswa'=>@$id_pegawai,'status_shp'=>1)))->num_rows();
		...
		*/
		...
		if($this->general_model->check_role($this->session->userdata('id_pegawai'),"mhs")){
			...
			if($cek_tanggal->row('start_date') == NULL AND $cek_tanggal->row('start_date') == NULL){
				...
				$btn_tambah = anchor(site_url($this->dir.'/pendaftaran_ujian'),'<i class="fa fa-plus fa-btn"></i> Pendaftaran Ujian Tesis', 'class="btn btn-success btn-flat" act="" title="Klik untuk tambah data"');
				}else{
		...
		if($this->general_model->check_role($this->session->userdata('id_pegawai'),"mhs")){
			$btn_tambah = anchor('#','<i class="fa fa-plus fa-btn"></i> Pendaftaran Ujian Proposal Tesis', 'class="btn btn-success btn-edit btn-flat" act="'.site_url($this->dir.'/pendaftaran_ujian/').'" title="Klik untuk tambah data"');
...
cek($dt->id_pemb_2);
cek($dt->judul_tesis);
cek($dt->id_ref_program_konsentrasi);
		die();*/
		$param1 =
			array(
				'tabel'=>'tesis',
    ...
    public function add_data($param=NULL){
       ...
       $from = array(
		...
		$data['form_data'] .= '<div class="col-lg-12">';
			...
			$dt_kom = $this->general_model->datagrab(array('tabel'=>'ref_tesis'));
...
*/
						...
						//$isi_file = $kom->id_ref_tipe_field;
						if($kom->id_ref_tipe_field == 2){
          					$isi_file = form_upload('berkas['.@$kom->id_ref_tesis.']',(@$dtnilai->num_rows()==NULL) ? NULL : $dtnilai->row('berkas'),'class="form-control" placeholder="Nilai"');

						}elseif($kom->id_ref_tipe_field == 3){
          					$isi_file = form_input('berkas['.@$kom->id_ref_tesis.']',(@$dtnilai->num_rows()==NULL) ? NULL : $dtnilai->row('berkas'),'class="form-control" placeholder="Nilai"');
...
';
		...
		$data['form_data'] .= '<input type="hidden" name="id_ref_tesis" class="id_ref_tesis" value="'.@$kom->id_ref_tesis .'"/>';
				$dtnilai = $this->general_model->datagrab(array('tabel'=>'mhs_tesis','where'=>array('id_tesis'=>$id, 'id_ref_tesis'=>$kom->id_ref_tesis)));
...
*/
	          			...
	          			$isi_file = form_upload('inp['.$kom->id_ref_tesis.']',($dtnilai->num_rows()==NULL) ? NULL : $dtnilai->row('inp'),'class="form-control" placeholder="Nilai"');
					}
	          	}else{
	          		$isi_file = '';
	          	}
		        ...
		        <tr>
		          ...
		          <th style="text-align:left">
		          <div class="col-lg-12" style="text-align:center;">'.form_hidden('id_ref_tesis[]', $kom->id_ref_tesis).$isi_file.'</div></th>';
		         if ($this->general_model->check_role($this->session->userdata('id_pegawai'),"pimp")) {

					$data['form_data'] .= '<th style="text-align:left"><i class="fa fa-check"></i></th>';
				}else{

					$data['form_data'] .= '<th style="text-align:left"><input  name="klm[]'.$kom->id_ref_tesis.'" '.$chk.' type="checkbox" value="'.$kom->id_ref_tesis.'" class="incheck" style="margin-top: -2px"></th>';
				}
    	...
    	$tgl_skrg = date('Y-m-d');
            if(empty($id_tesis)) {
                ...
                if(empty($cek_prop->id_tesis)) {
                	...
                	$id_prop = $this->general_model->save_data('tesis',array('id_mahasiswa' => $id_pegawai,'id_pemb_1' => $id_pemb_1,'id_pemb_2' => $id_pemb_2,'judul_tesis' => $judul_tesis,'id_ref_program_konsentrasi' => $id_ref_program_konsentrasi));
    ...
    function save_verifikasi(){
    	if($this->general_model->check_role($this->session->userdata('id_pegawai'),"pimp")){
			...
			$parx = array(
					...
					$parx['where'] = array('id_mahasiswa'=>$id_mahasiswa,'id_pemb_1'=>$id_pemb_1,'id_pemb_2'=>$id_pemb_2);
