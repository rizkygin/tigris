	...
	public function list_data($search=NULL, $offset=NULL) {
		...
		if (!empty($search_key)) {
			$fcari = array(
				...
				'id_pegawai' 		=> $search_key,
			);	
			$data['for_search'] = $fcari['judul_tesis'];
			$data['for_search'] = $fcari['id_pegawai'];
		} else if ($search) {
			$fcari=un_de($search);
			$data['for_search'] = $fcari['judul_tesis'];
			$data['for_search'] = $fcari['id_pegawai'];
		...
		$from = array(
			'jadwal_ujian a' => '',
			'proposal_tesis b' => array('b.id_pegawai = a.id_mahasiswa','left'),
			'peg_pegawai c' => array('b.id_pegawai = a.id_mahasiswa','left'),
			'ref_program_konsentrasi d' => array('c.id_ref_program_konsentrasi = b.id_program_studi','left')
		);
		$select = 'a.*,b.nama as nama_mahasiswa,b.id_pegawai,c.nama_program_konsentrasi';
		$where = array('a.status_pt'=>1);
		$config['per_page']		= '5';
...
*/
				...
				$rows[] = 	(($cek_jadwal > 0)?@$cek_data_jadwal->tanggal_ujian:'');
    ...
    public function tambah_jadwal($param=NULL){
       ...
       $from = array(
		...
		$data['form_data'] .= '<br><br><h1>Jadwal Ujian Proposal Tesis</h1><hr>';
			...
			$data['form_data'] .= '<p><label>Tanggal dan Jam</label>';
			$data['form_data'] .= form_input('tanggal_ujian', !empty(@$dt_row->tanggal_ujian)?@$dt_row->tanggal_ujian:null,'class="form-control" id="tanggal_ujian" placeholder="tangal Ujian dan jam" required');
		...
		$data['script'] = '$("#tanggal_ujian").datetimepicker();initMce();';
...
die();*/
    ...
    // Line break
        ...
        $pdf->SetTextColor(0,0,0);
        $pdf->SetX(25);
        $pdf->Cell(20,8,'Pada hari ini,    '.konversi_tanggal('D',substr($dt_row->tanggal_ujian,0,10)).'  tanggal    '.konversi_tanggal('j',substr($dt_row->tanggal_ujian,0,10)).'   Bulan    '.konversi_tanggal('M',substr($dt_row->tanggal_ujian,0,10)).'  Tahun    '.konversi_tanggal('Y',substr($dt_row->tanggal_ujian,0,10)).'    telah dilaksanakan');
        ...
        $html='<table border="1">
<tr>
<td width="40" height="40">No</td>
<td width="380" height="40" bgcolor="#D0D0FF">Nama Penguji</td>
<td width="200" height="40" bgcolor="#D0D0FF">Jabatan</td>
<td width="150" height="40" bgcolor="#D0D0FF">Tanda Tangan</td>
</tr>
<tr>
<td width="40" height="60">1</td>
<td width="380" height="60" bgcolor="#D0D0FF">'.$dt_row->pemb_1.'</td>
<td width="200" height="60" bgcolor="#D0D0FF">Pembimbing</td>
<td width="150" height="60" bgcolor="#D0D0FF">1.............................</td>
</tr>
<tr>
<td width="40" height="60">12</td>
<td width="380" height="60" bgcolor="#D0D0FF">'.$dt_row->penguji_1.'</td>
<td width="200" height="60" bgcolor="#D0D0FF">Anggota</td>
<td width="150" height="60" bgcolor="#D0D0FF">.............................2</td>
</tr>
<tr>
<td width="40" height="60">3</td>
<td width="380" height="60" bgcolor="#D0D0FF">'.$dt_row->penguji_2.'</td>
<td width="200" height="60" bgcolor="#D0D0FF">Anggota</td>
<td width="150" height="60" bgcolor="#D0D0FF">3............................</td>
</tr>
</table>';
...
$pdf->WriteHTML($html);
        ...
        $pdf->SetX(64);
        $pdf->SetFont('Times','',12);
        $pdf->MultiCell(120,8,''.$dt_row->penguji_1.'');
        ...
        $pdf->SetTextColor(0,0,0);
        $pdf->SetX(140);
        $pdf->Cell(20,8,''.$dt_row->penguji_1.'');
...
');
        ...
        $pdf->SetTextColor(0,0,0);
        $pdf->SetX(140);
        $pdf->Cell(20,8,''.$dt_row->penguji_2.'');
...
');
        ...
        $pdf->SetX(64);
        $pdf->SetFont('Times','',12);
        $pdf->MultiCell(130,8,''.$dt_row->pemb_1.'');
...
');
    ...
    // Line break
        ...
        $pdf->SetX(-105);
        $pdf->SetTextColor(900,900,900);
         $pdf->Image(base_url()."assets/images/corp.png",0,0,210,0);
        ...
        $pdf->SetTextColor(0,0,0);
        $pdf->SetX(55);
        $pdf->Cell(20,8,'Hari/Tanggal  : '.tanggal($dt_row->tanggal_ujian).' Fakultas yang akan diselenggarakan pada :');

        $pdf->SetFont('Times','',12);
        $pdf->Ln(7);
        $pdf->SetTextColor(0,0,0);
        $pdf->SetX(55);
        $pdf->Cell(20,8,'Pukul              : '.tanggal($dt_row->tanggal_ujian).' - selesai WIB');
