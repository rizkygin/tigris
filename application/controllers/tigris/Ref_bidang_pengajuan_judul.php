...
class Ref_bidang_pengajuan_judul extends CI_Controller{
    var $dir = 'tigris/Ref_bidang_pengajuan_judul';
    ...
    public function list_data($search=NULL, $offset=NULL) {
		...
		foreach ($nav->result() as $rowx) {
			$awal[] = array($rowx->id_bidang_pengajuan_judul,$rowx->urut);
		}
		...
		if ($dtjnsoutput->num_rows() > 0) {
			...
			foreach ($dtjnsoutput->result() as $row) {
				...
				$btn_down = ($m+1 < $nav->num_rows()) ? anchor('tigris/Ref_bidang_pengajuan_judul/urut/'.in_de(array('id1' => $row->id_bidang_pengajuan_judul,'no1' => $row->urut,'id2' => $awal[$m+1][0],'no2' =>  $awal[$m+1][1])),'<i class="fa fa-arrow-down"></i>') : ' &nbsp; ';
					$btn_up = ($m > 0) ? anchor('tigris/Ref_bidang_pengajuan_judul/urut/'.in_de(array('id2' => $row->id_bidang_pengajuan_judul,'no2' => $row->urut,'id1' =>  !empty($urutkan[0])?$urutkan[0]:@$awal[0],'no1' => !empty($urutkan[1])?$urutkan[1]:@$awal[1])),'<i class="fa fa-arrow-up"></i>') : ' &nbsp; ';
					$urutkan = array($row->id_bidang_pengajuan_judul,$row->urut);
		...
		$title = 'Referensi Urutan Bidang Penagjuan Judul';
    ...
    public function add_data($param=NULL){
    	$o = un_de($param);
    	$id= @$o['id_bidang_pengajuan_judul'];
        $data = array(
            'button' => 'Tambah',
            'action' => site_url('tigris/Ref_bidang_pengajuan_judul/save_aksi'),

        'id_bidang_pengajuan_judul' => set_value('id_bidang_pengajuan_judul'),
		);
        $from = array(
			'ref_bidang_pengajuan_judul a' => ''
		);
		...
		$data['form_data'] = '';
		$data['form_data'] .= '<input type="hidden" name="id_bidang_pengajuan_judul" class="id_bidang_pengajuan_judul" value="'.$id .'"/>';
    ...
    function delete_data($code) {
		$sn = un_de($code);
		$id_bidang_pengajuan_judul = $sn['id_bidang_pengajuan_judul'];
		$del = $this->general_model->delete_data('ref_bidang_pengajuan_judul','id_bidang_pengajuan_judul',$id_bidang_pengajuan_judul);
		if ($del) {
    ...
    function save_aksi(){
    	...
    	$id_bidang_pengajuan_judul = $this->input->post('id_bidang_pengajuan_judul');
    	...
    	$tgl_skrg = date('Y-m-d');
     	$u = $this->general_model->datagrab(array(
            ...
            if(empty($id_bidang_pengajuan_judul)) {
                ...
                if(empty($cek_prop->id_bidang)) {
                	$id_prop = $this->general_model->save_data('ref_bidang_pengajuan_judul',array('id_ref_bidang' => $input['id_bidang'],'urut' => $urut));	
            	...
            	$par = array(
					...
					$par['where'] = array('id_bidang_pengajuan_judul'=>$id_bidang_pengajuan_judul);
	...
	function urut($par) {
		...
		$param1 =
			...
			$param1['where'] = array('id_bidang_pengajuan_judul'=>$o['id1']);
		...
		$param2 =
			...
			$param2['where'] = array('id_bidang_pengajuan_judul'=>$o['id2']);
	...
	function on($par) {
		...
		$param1 =
			...
			$param1['where'] = array('id_bidang_pengajuan_judul'=>$o['id_bidang_pengajuan_judul']);
			$this->general_model->simpan_data($param1);
			redirect($this->dir);
	}
}
