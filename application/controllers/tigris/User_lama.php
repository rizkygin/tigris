...
class User extends CI_Controller {
	 ...
	 * $indie 
	 *		TRUE = Tambah User Independen di controller User
	 *		FALSE = Non-Akfif Tambah User
	 * $app
	 *		1 = Aplikasi Tunggal
	 *		0 = Seluruh Aplikasi
	 * $dir	
	...
	function list_data($search = null, $offset = null){
		...
		$data['breadcrumb'] = array('' => $this->in_app, $this->dir.'/User' => 'User');
		...
		$config['base_url']	= site_url($this->dir.'/User/list_data/'.in_de($vse));
		...
		$dtUser	= $this->general_model->datagrab(array(
			'tabel' => $from,
		...
		if($dtUser->num_rows() > 0){
			...
			foreach($dtUser->result() as $row){
				...
				$link1 = '<a href="#" act="'.site_url($this->dir.'/User/add_user/'.(in_de($vse).'~'.$offset).'/'.$row->id_pegawai).'" class="btn btn-xs btn-warning btn-edit"><i class="fa fa-pencil"></i></a>';
				$link2= ' <a href="'.site_url($this->dir.'/User/reset_User/'.$row->id_pegawai).'" class="btn btn-xs btn-primary btn-reset" title="Reset Password" msg="Yakin untuk mereset password User? <br>(Password standar adalah <b>qwerty</b>)"><i class="fa fa-repeat"></i></a>';
				...
				$rows[] = $link2;
				if ($this->indie) {
					$rows[] = anchor($this->dir.'/User/delete_data/'.$row->id_pegawai.'/'.(in_de($vse).'~'.$offset),'
			...
			$tabel = '<div class="alert">Tidak ada data User</div>';
		}
		...
		$data['extra_tombol'] = form_open($this->dir.'/User','id="form_search"').
		...
		if ($this->indie) {
			$data['tombol'] = anchor('','<i class="fa fa-plus fa-btn"></i> Tambah User','
				class="btn btn-success btn-edit" 
				act="'.site_url($this->dir.'/User/add_user').'"
				title="Tambah User"');
		}
	...
	function add_user($offs = null,$id = null) {
	...
	function save_User(){
		...
		$this->session->set_flashdata('fail','Username sudah tersedia!');
			...
			if (count($role) > 0) {
				...
				if (!empty($id_pegawai)) $psn = 'Ubah User berhasil dilakukan ... ';
				else $psn = 'User berhasil ditambahkan ... '.(($this->indie)?'<br>Username : <strong>'.$usr.'</strong><br/>Password : <strong>'.$usr.'</strong>':null);
		...
		redirect($this->dir.'/User/list_data/'.str_replace('~','/',$offs));
		
	}
	
	function reset_User($id=null){
		$this->general_model->save_data('peg_pegawai',array('password' => md5('qwerty')),'id_pegawai',$id);
		$this->session->set_flashdata('ok', 'Password User berhasil di reset');
		redirect($this->dir.'/User');
	}
