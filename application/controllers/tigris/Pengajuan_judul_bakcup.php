...
class Pengajuan_judul extends CI_Controller {
	...
	public function list_data($search=NULL, $offset=NULL) {
		...
		$from = array(
			...
			'ref_program_konsentrasi c' => 'c.id_ref_program_konsentrasi = a.id_ref_program_konsentrasi',
		);
		$select = 'a.*,a.id_ref_program_konsentrasi as s,a.judul_tesis as xx,b.*,c.*,c.id_ref_program_konsentrasi as ss';
		...
		if ($dtjnsoutput->num_rows() > 0) {
			...
			foreach ($dtjnsoutput->result() as $row) {
				...
				if (!in_array($offset,array("cetak","excel")) && $this->general_model->check_role($this->session->userdata('id_pegawai'),"mhs")) {
					...
					$ubah = anchor('#','<i class="fa fa-pencil"></i>', 'class="btn btn-xs btn-warning btn-edit btn-flat" act="'.site_url($this->dir.'/add_data/'.in_de(array('id_mahasiswa'=>$row->id_mahasiswa,'id_pengajuan_judul'=>$row->id_pengajuan_judul))).'" title="Edit Data..."');
				...
				if ($this->general_model->check_role($this->session->userdata('id_pegawai'),"akad") OR $this->general_model->check_role($this->session->userdata('id_pegawai'),"perp") OR $this->general_model->check_role($this->session->userdata('id_pegawai'),"keua")) {
					...
					$verifikasi1 = anchor('#','<i class="fa fa-list"></i>', 'class="btn btn-xs btn-primary btn-edit btn-flat" act="'.site_url($this->dir.'/verifikasi/'.in_de(array('id_mahasiswa'=>$row->id_mahasiswa,'id_pengajuan_judul'=>$row->id_pengajuan_judul))).'" title="Verifikasi data..."');
				...
				if ($this->general_model->check_role($this->session->userdata('id_pegawai'),"pimp")) {
					...
					$verifikasi2 = anchor('#','<i class="fa fa-list"></i>', 'class="btn btn-xs btn-primary btn-edit btn-flat" act="'.site_url($this->dir.'/verifikasi/'.in_de(array('id_mahasiswa'=>$row->id_mahasiswa,'id_pengajuan_judul'=>$row->id_pengajuan_judul))).'" title="Verifikasi data..."');
...
/*
		...
		//cek($cek_pengajuan_judulx);*/
/*cek($cek_pengajuan_judul2);*/
		if($this->general_model->check_role($this->session->userdata('id_pegawai'),"mhs") && $cek_pengajuan_judul > 0 AND $cek_pengajuan_judul2 == 0){
    ...
    public function add_data($param=NULL){
		...
		$data['form_data'] .= '<div class="col-lg-6">';
			...
			$data['form_data'] .= '<p><label>Program Konsentrasi</label>';
			$data['form_data'] .= form_dropdown('id_ref_program_konsentrasi', $cb_tipe,@$dt->id_ref_program_konsentrasi,'class="form-control combo-box" style="width: 100%"');
			$data['form_data'] .= '</div>';
		$data['form_data'] .= '<div class="col-lg-6">';
			$data['form_data'] .= '<p><label>Pembimbing 1</label>';
			$data['form_data'] .= form_dropdown('id_pembimbing_1', $cb_pembimbing1,@$dt->id_pembimbing_1,'class="form-control combo-box"  required style="width: 100%"');
			$data['form_data'] .= '<p><label>Pembimbing 2</label>';
			$data['form_data'] .= form_dropdown('id_pembimbing_2', $cb_pembimbing2,@$dt->id_pembimbing_2,'class="form-control combo-box" style="width: 100%"');
			/*
			$data['form_data'] .= '<p><label>Keterangan</label>';
			$data['form_data'] .= form_input('keterangan_seminar_penelitian', @$dt->keterangan_seminar_penelitian,'class="form-control" placeholder="Nama Pengajuan Judul" required');*/
			if(!empty($id)){
			$data['form_data'] .= '</div>';
		$data['form_data'] .= '<div class="col-lg-12">';
			...
			$dt_kom = $this->general_model->datagrab(array('tabel'=>'ref_pengajuan_judul'));
...
*/
	          			$isi_file = '<a href="'.base_url('/uploads/'.$dtnilai->row('berkas')).'" class="fancybox" >'.$dtnilai->row('berkas').'</a>';
					}else{
...
';
		...
		$data['form_data'] .= '<input type="hidden" name="id_ref_pengajuan_judul" class="id_ref_pengajuan_judul" value="'.@$kom->id_ref_pengajuan_judul .'"/>';
				$dtnilai = $this->general_model->datagrab(array('tabel'=>'mhs_pengajuan_judul','where'=>array('id_pengajuan_judul'=>$id, 'id_ref_pengajuan_judul'=>$kom->id_ref_pengajuan_judul)));
...
*/
		        ...
		        <tr>
		          ...
		          <th style="text-align:left">
		          <div class="col-lg-12" style="text-align:center;">'.form_hidden('id_ref_pengajuan_judul[]', $kom->id_ref_pengajuan_judul).form_hidden('id_berkas[]', $kom->id_ref_pengajuan_judul).$isi_file.'</div></th>';
		         if ($this->general_model->check_role($this->session->userdata('id_pegawai'),"pimp")) {
    	...
    	$tgl_skrg = date('Y-m-d');
            if(empty($id_pengajuan_judul)) {
                ...
                if(empty($cek_prop->id_pengajuan_judul)) {
                	...
                	$id_prop = $this->general_model->save_data('pengajuan_judul',array('id_mahasiswa' => $id_pegawai,'id_pembimbing_1' => $id_pembimbing_1,'id_pembimbing_2' => $id_pembimbing_2,'judul_tesis' => $judul_tesis,'id_ref_program_konsentrasi' => $id_ref_program_konsentrasi));
